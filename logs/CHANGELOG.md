...
## 2026-09-09(その27)—— ADR-069。人間が 4 束すべてを決め、4 件とも実装まで終えた

- **★★人間が 4 束を決定した(ADR-069)。**決定材料はその25 / その26 が積み上げたものである
  (`plans/PLAN-019` §10.10.2 / §10.10.4.7 / §10.11.6 / §10.12)。
  **提案 エージェント (Opus) / 採択 人間**(`CLAUDE.md` §8 / ADR-039 決定3)
  - **★F112 = 案 (a)**(差し替える)/ **★F113 = 案 (a)**(移し替えを明示)/
    **★F114 = 並列化だけ先に実装**(実行先は決めない)/ **実装判断 4 件 = 全件承認**
- **★F112 の実装(決定1)。★F112 は閉じた**
  - **`configs/power_sim.yaml` / `power_sim_smoke.yaml` に `effect.eta`(丸める前の 12 セル)を
    新設し、DGP の真値にした。**復元値の出所は §6.5 そのもの(表の上の 1 文の `mu` / `a` / `b` と
    「勾配 g」の列)であり、全数値は `test_power_sim.py` の `RECOVERED_OFFSETS` が固定している
  - **`effect.rule_rate` は消さず、「`eta` を丸めたものと一致するか」の門にした。**
    片方だけ動かすと `power_sim.py` が止まる(§6.5 の表と DGP が離れたことの検出器)
  - **非加法性 RMS が 0.3605 → 0.352767**(§6.5 が載せる 0.353 と 3 桁で一致)。
    **★P0 は厳密に 0 になり、`sigma = 0` の行が α の較正として読める**
  - **`build_profile` を書き直した**(`inv_logit` / `_require_shape` を新設。`logit` は廃した)。
    **`eta` は必須にした** —— 表から作り直す既定の経路は残さない(skill `code-style` §5)
- **★F114 の並列化(決定3)。★「どこで回すか」は決めていない**
  - **`shard_jobs` / `run_fits` を新設。**当てはめを連続ブロックで切り、複数の `Rscript` を
    同時に走らせる。**R の起動は 1 並列単位につき 1 回のまま**(起動と `lme4` の読み込みが高い)。
    **1 本でも落ちたら止める**(当てはめの失敗を黙って数えない。`CLAUDE.md` §7)
  - **`fit.n_workers`(既定 1 = 逐次)と `--workers`。★実験パラメータではなく実行資源である** ——
    表は親が単一の `rng` から決まった順で書き、当てはめは表ごとに決定的だからである
  - **★実測で確かめた**: **smoke を `--workers 1` と `--workers 4` で回し、
    `power_sim_summary.json` が `n_workers` 以外 完全に一致した**(結果は並列数に依らない)。
    **★smoke の `power` の値は結果として読まない**(`configs/smoke.yaml` と同じ扱い)
  - **`describe_plan` のコスト行を ★F114 の実測に差し替えた** ——
    **992 コア時 / 16 並列で 62 時間**(定数 `SECONDS_PER_PAIR_N_ITEM_48 = 297.6`)。
    **旧「約 9 時間」は F50 からの掛け算で 7 倍甘かった**
- **★F113 の反映(決定2)。`Documents/05_STATISTICS.md` §5(事前登録の本体)**
  - **Δ 4 行(#2〜#5)の出所欄に打ち消し線を引き、「★順6 からの移し替え」の節を新設した**
    (`CLAUDE.md` §2。**旧記述は打ち消し線 + 理由 + 日付で残る**)
  - **食い違い 3 つを明示**((i) DV が違う /(ii) 条件が無い /(iii) タスク型の範囲が違う)。
    **順6 が産むのは「素のモデル・T2 のみ・`correct_rate` のテンプレート間分散」1 個だけである**
  - **★移し替えの 3 仮定を弱点として書いた**(従属変数 / 段階 / タスク型)。
    **★3 仮定は検証されないまま凍結に入る**(FT 後の `rule_rate` はパイロット= 段階 E まで出ず、
    段階 E は凍結の後にある)。**論文の弱点欄に残す**
  - **★1 行目(勾配が平行)は順6 に依存しない**と明記した(§6.5 の P1 から決まる)。
    **★順6 の GPU は依然として要る**(Go/No-Go #0〜#3 は順6 でしか出ない)とも明記した
- **テスト**: **`pytest code/tests -q` = 884 → 898 passed**
  - **旧 ★F112 の 3 件は決定後の姿へ書き直した**(丸めが RMS を膨らませるという事実そのものは
    `test_rounding_the_recovered_logits_is_what_inflates_the_rms` が今も測っている)。
    **差し替え前の 2 定数は「何が直ったか」の記録として残した**(`CLAUDE.md` §2)
  - **並列実行の回帰テスト 7 件**(シャードが当てはめを過不足なく 1 回ずつ覆う / 順序を保つ /
    `n_workers` を変えても仕事が変わらない / R の起動回数 / 1 本の失敗が伝わる / コスト行が実測)
  - **★F113 の文書回帰テスト 2 件** —— **出所欄が「順6 の実測」に戻ったら落ちる。**
    ★F90 / ★F104 と同じ形の穴の再発を機械で止める
- **`logs/OPEN-ITEMS.md`**: ★F112 / ★F113 / ★F114 / 実装判断 4 件に打ち消し線 + ADR-069 + 日付。
  **★新しく「★F114 の実行先」を索引に足した**(並列化は済んだが実行先は未決)
- **`STATE.md`**: 400 行を保ったまま更新。旧ブロック 3 つを `logs/STATE-ARCHIVE.md` §4 / §7 / §8 へ移した
  (ADR-063 運用規約1。**捨てていない**)。**★運用規約5 の上限表記を ADR-063 決定2 に合わせた** ——
  **その26 が「文書 400 行と回帰テスト 700 行が食い違っている」と申し送ったが、食い違いではない。**
  **ADR-063 決定2 が同じ文で「目標 400 行 / 60 KB」と「テストが落ちるのは 700 行 / 90 KB」を宣言している**
- **★やっていないこと**: **本実行を回していない**(`dgp.n_item` / `s2_item` / `s2_tmpl` が null)/
  **★F114 の実行先を決めていない** / **`n_item = 96` のベンチを完走していない** /
  **文献を 1 件も探していない** / **凍結タグを打っていない** / **GPU を起動していない**(GPU 時間 0)


## 2026-09-09(その28)— PLANNER / CRITIC (Opus)。★F104 の決定材料(**決定 0 件**)

**指示は `logs/HANDOFF.md`(その27)の「★F104 の決定材料を作る。決定は人間」。**
**★F104 には事実(§10.4)はあったが案が無く、§10.8 の記入欄が空いていた。**
**GPU 時間 0。`results/` は空のまま。凍結タグは打っていない。文献は 1 件も探していない。**
**ADR-067 / ADR-068 / ADR-069 は 1 文字も触っていない**(蒸し返し禁止。HANDOFF の指示)。

- **★問いの形が変わった。**★F104 は「順6 の `correct_rate` から移し替えてよいか」だと思われていたが、
  **検査3 で 2 つの事実が出て、問いが 2 本に割れた**(`plans/PLAN-019` §10.13.0)
  - **★F115 —— 順6 の形では `s2_item` の同定が効かない。**順6 は 1 本の run(素のモデル)で、
    T2 以外の項目は **1 回ずつ**しか観測されない。**1 項目 1 観測の Bernoulli では
    profile 対数尤度が完全に平ら**(検査3 [A1]: `s2_item` = 0 〜 16 で対数尤度差 0、
    最大でも浮動小数の丸め `2.8e-14`)。**タスク5 の反復 3 回は温度 0 であり、
    一致する限り対数尤度は単調に増える**(検査3 [A2]: −775.9 → −301.3。内点の MLE が無い)。
    **★「移し替えてよいか」以前に、移し替える元の量が立つ保証が無い。★F90 と同じ形の穴である**
  - **★F117 —— `task:coverage` の交互作用へ直接入るのは `s2_item` だけである。**
    `n_item = 48` で `s2_item` = 0.5 / 1.0 / 2.0 / 4.0 のとき、交互作用に乗る非加法性 RMS が
    **0.098 / 0.139 / 0.195 / 0.276**(採択済み P1 = 0.352767 の **28% / 39% / 55% / 78%**)。
    **`s2_tmpl` は 0 〜 8 のどこでも厳密に 0、対照として測った `sigma`(seed)も 0。**
    理由は構造 —— **`w[template]` は巡回割当で被覆水準間で同じ値になり差分で消え**
    (ADR-069 決定4 (3))、**`u[s, cov]` は `task` の添字を持たない**(ADR-066 決定4 (d))
  - **★賭け金**: **`s2_item` はシード数では縮まない**(`v[item]` は 10 シード共有)。
    **縮むのは `n_item` に対してのみで `1/sqrt(n_item)`**(検査3 [D])。**`n_item` は `M*` 待ちである**
  - **★F116** —— 順6 が産む `s2_tmpl` は **T2 の 5 category** のみ。当てはめが使う水準は
    **`category` 10 水準**(ADR-062 決定2)で **`template` は `task` に入れ子**、**T1 は 1 水準**。
    5 水準からの分散推定の相対標本 SD は **0.707**(算術)
  - **★F118** —— **`s2_tmpl` に 0 に近い値を置くのは安全な既定ではない。**
    縮退カスケードの第1段が `template` である(ADR-065 決定2)。**発火率は §6.3 手続き3b の報告対象**。
    **★これは読解であって測っていない**(`glmer` を当てていない)
  - **★F119** —— 順6 が産むテンプレート間分散は 1 個しかなく、**ADR-069 決定2 が既に Δ の
    測定ノイズ床に割り当てている**。`s2_tmpl` にも使うと 1 個の実測が 2 つの用途を持つ
- **★検査3 を足した**(`plans/PLAN-019-check3/`。`README.md` / `item_template_source_check.py` / `run.log`)。
  **★合成データと算術である。実験結果ではない**(`CLAUDE.md` §2)。
  **`numpy` のみ・R 不要・1 分以内。**`plans/PLAN-019-check1` / `check2` と同じ扱いである
  - **★対照行を必ず入れた** —— 同じ物差しを seed 項にも当てて 0 であることを示し、
    **「seed の分散は検出力に効かない」と誤読されない形にした**(`sigma` は二項抽出の水路で効き、
    それは検査2 が測った F108。**本検査はその水路を持たない**)
- **`plans/PLAN-019-validity-decisions.md` §10.13 を新設**(決定材料。**決定 0 件**)。
  §10.13.0(1 ページ要約)/ §10.13.1(F115 〜 F119)/ §10.13.2(`s2_item` の 5 案)/
  §10.13.3(`s2_tmpl` の 3 案)/ §10.13.4(書き換わるもの)/ **§10.13.5(記入欄。空)** / §10.13.6(限界)
  - **★決定を 4 行に割った**: **F104-0**(2 つに分けてよいか)/ **F104-a**(`s2_item` の取得元)/
    **F104-b**(`s2_tmpl` の取得元)/ **F104-c**(値そのもの)
  - **★案 A(順6 からの移し替え)には「推奨しない」を付けた**(★F115。ADR-057 / F90-1 案 D の前例に倣う)。
    **案 B(格子)と案 C(1 点)はどちらも推していない** —— **差はコストであり、それは人間の判断である**
  - **★コストを算術で出した**: 現行 24,000 本 = **62 時間**(16 並列。★F114 の実測)。
    **`s2_item` を k 点掃くと k 倍** —— k = 3 で **186 時間**、4 で **248**、6 で **372**。
    **★F114 の実行先(未決)に直接効く**
  - **★値そのものは提案していない**(ADR-053 決定2 と同じ立場。`CLAUDE.md` §8)
- **回帰テスト 2 件を足した**(`code/tests/test_power_sim.py`)。**★F117 が乗っている
  `draw_frame` の構造を機械で固定する** —— `v[item]` が全シードで共有されること /
  同じ項目添字にどの被覆水準でも同じテンプレートが付くこと。
  **★片方は変異させて落ちることを確かめた**(seed ループの内側で `v` を引く形に変えると落ちる)。
  **`pytest code/tests -q` = 898 → 900 passed**
- **`logs/OPEN-ITEMS.md`**: ★F104 の行を「決定材料ができた」に差し替え、記入欄を §10.13.5 へ送った。
  §10.8 の F104 行と §10 の冒頭注も追随させた
- **★やっていないこと**: **`glmer` を 1 本も当てていない**(**検出力そのものは 1 つも出していない**)/
  **`Documents/05_STATISTICS.md` と `configs/power_sim.yaml` を触っていない**(どちらも事前登録の
  本体・入力であり、人間の決定が要る)/ **`dgp.s2_item` / `s2_tmpl` / `n_item` は `null` のまま** /
  **本実行を回していない** / **GPU を起動していない** / **文献を 1 件も探していない**


### 2026-09-09(その29)— PLANNER / CRITIC。★`θ` の決定材料を作った(★F120 / ★F121 を開いた)

**依頼**: 「順5 を動かすための `θ` の決定材料を先に作る」(ユーザー。その28 の見立てのとおり)。
**決定は 0 件。**`Documents/` と `configs/` は 1 文字も触っていない。**GPU 時間 0。実験は 1 件も回していない。**

- **`plans/PLAN-020-theta-decision.md` を新設**(決定材料。**決定 0 件**)。
  §0(1 ページ要約)/ §1(何を決めるか)/ §2(凍結済みで触らないもの)/ §3(★θ-0)/
  §4(検査1)/ §5(検査2)/ §6(案)/ **§7(記入欄。空)** / §8(限界 5 件)
- **★決定が 1 行から 4 行に割れた**: **θ-0**(いつ決めるか)/ **θ-a**(値)/
  **θ-b**(`M*` が 100〜133 のときの分岐)/ **θ-c**(殻の正答率を報告するか)
- **★F120 —— 主軸 3 水準目の母集団は `(M* − 99)²` しかない**(`plans/PLAN-020-check1/`。
  **組合せ論の計数。実験結果ではない**)。`MAIN_COVERAGE_LEVELS` は
  `id` / `interp` / **`extrap_magnitude`**(`code/data_gen/pool.py:59`)であり、
  `extrap_magnitude` は `a > 99` かつ `b > 99`(両側正)。
  **`M* = 100` で 1 組 / 110 で 121 組 / 125 で 676 組。**セル表の要求は **520 組(carry 層 240)**。
  **両方を満たす最小の `M*` は 134、格子上の最小の成立点は 150。**
  **★ADR-041 決定3 規則5 は `M* < 100` にしか分岐が無く、100 / 110 / 125 はその外側にある** ——
  規則どおり進むと `fill_cells` が**順5 の後・凍結の直前に**例外で止まる
- **★F121 —— `correct_rate(M)` は殻ではなく `R(M)` 全体の累積平均である**(**仮定を置いた算術**)。
  `magnitude_sweep.build_items` は `[-M, M]²` から一様に引くので、**`M = 100` では引かれる組の
  98.0% が主域の中にある。**仮定シナリオ S1(崖が 99)では **θ = 0.50 〜 0.90 のどれを選んでも
  `M*` が 100 / 110 / 125 に落ちる**(規則5 は 1 度も発火しない)。
  **そこで組まれる外挿セルは素のモデルの正答率が 0 の項目で埋まる** ——
  **`plans/PLAN-001` §4.1.1 が「設計段階で潰れる」と書いた懸念6 そのものである**
  - **★殻の正答率は追加の GPU 時間 0 で取れる。**`predictions/` の 1 行に `operands` が残る
    (`code/eval/run.py:681`)。**解析側の実装だけである**(§5.4。**まだ実装していない**)
- **★θ-0(いつ決めるか)が記録の中で食い違っていた**(PLAN-020 §3)。
  `plans/PLAN-001` §4.1.1 と `logs/OPEN-ITEMS.md` は「**掃引表を見てから**」、
  ADR-041 文脈 / ADR-057 リスク欄 / `STATE.md` は「**順5 の前に**」と書いている。
  **どの ADR も決定としては書いていない。**したがって θ-0 は未決である
- **回帰テスト 6 件を足した**(`code/tests/test_design_facts.py`)。格子点 4 点の母集団 /
  100 と 110 で C6 が組めないこと / 最小の成立点が 134 であること。
  **`pytest code/tests -q` = 900 → 906 passed**
- **★やっていないこと**: **θ の値を提案していない**(規範的な線引きであり `CLAUDE.md` §8 の対象)/
  **`Documents/` と `configs/` を触っていない** / **殻ごとの正答率を実装していない** /
  **ポッドを起動していない** / **文献を 1 件も探していない**
- **影響を受けたファイル**: `plans/PLAN-020-theta-decision.md`(新規)/
  `plans/PLAN-020-check1/`(新規。README / 2 スクリプト / 2 ログ)/
  `code/tests/test_design_facts.py` / `logs/OPEN-ITEMS.md` / 本ファイル / `STATE.md`

- **★追記(同セッション)**: **`plans/PLAN-020` §6.5 を新設した** ——
  ユーザーの問い「元々の理想(見たことがある問題 vs ない問題)に近いのはどれか」への整理。
  **理想の核は `id` vs `interp` であり、この 2 水準は難しさが揃っている**(違いは FT データに
  入っていたかどうかだけ)。**`extrap_magnitude` は「距離」を足すための 3 水準目**であり、
  素のモデルが解けない域を含むと理想が汚れる。**θ-c C2 が最も忠実、θ-b B1 が核を完全に保つ、
  θ-b B2 と θ-c C3 は理想の否定**、**θ-a(値)はほぼ無関係**(θ は「解けるか」の線であって
  「見たか」の線ではない)。**★`id` vs `interp` は `θ` に一切依存しない** ——
  元々の問いは本 PLAN の決定がどう転んでも生き残る。**決定は引き続き 0 件。**
- **セッション引き継ぎ**: `logs/HANDOFF.md` を上書き(次セッションは ADR-070 を書く役)。
  **コンテキスト超過(約 209k / 閾値 140k)で切った。**`STATE.md` は 409 行(目標 400 / 閾値 700)

---

## 2026-09-09(その30)

### docs(adr): ADR-070 —— 人間が `θ` の 4 行を採択した(A2 / 0.70 / B1 / C2)   [actor: PLANNER]

- **ユーザーが `plans/PLAN-020` §7 の 4 行すべてを埋めた**(2026-09-09):
  **θ-0 = A2**(順5 の前に `θ` を確定し、`θ ± 0.05` での `M*` を副次として事前登録)/
  **θ-a = 0.70** / **θ-b = B1**(C6 が組めないなら外挿を取り下げ df 6 → 3)/
  **θ-c = C2**(**`M*` の判定そのものを殻の正答率に変える**)
- **`logs/DECISIONS.md` に ADR-070 を追記した**(提案 エージェント (Opus) / 採択 人間。ADR-039 決定3)。
  **実験は 1 件も回っていない**(`results/` は空、GPU 時間 0、事前登録は未凍結)ので、
...
- **反映**:
  - `plans/PLAN-001` §4.1.1 に 2 つの改訂ブロックを入れた(**打ち消し線 + 理由 + 日付**。`CLAUDE.md` §2)。
    手続き 2・3 の `correct_rate` が**殻**に変わったこと / 規則5 の条件が
    **「C6 が組めない値」**に広がったこと。**ADR-070 が正本**
  - `configs/exp_phase1_main.yaml` の `eval.magnitude_sweep` に
    **`theta: 0.70` / `theta_secondary: [0.65, 0.75]` / `shell_definition: null` / `shell_n_items: null`** を追加。
    **`extrapolation_radius`(`M*`)は引き続き `null`**
  - `plans/PLAN-020` §7 の記入欄を埋めた / `logs/OPEN-ITEMS.md` の `θ` 行に打ち消し線
- **★新しく開いた人間待ちが 3 件ある**(`logs/OPEN-ITEMS.md` が正本)。**C2 は測定を一意に定めない**:
  1. **殻の定義** —— `R(M) \ R(1つ前の格子点)` か `R(M) \ R(99)` か
  2. **殻あたりの項目数と抽出経路** —— (a) 現行の抽出のまま `predictions/` を殻で切り直す(**追加 GPU 0**)か、
     (b) **殻から 200 件引く**(ADR-041 決定5 の変更)か。
     **★(a) は `M = 100` の殻に 1,000 項目中およそ 20 項目しか落ちず SE 約 0.10。**(b) なら SE 約 0.014
  3. **`θ = 0.70` の根拠** —— ADR-041 決定2 が `#9` とは別に書くことを要求している。**値は確定**
- **テスト**: `pytest code/tests -q` = **906 passed**(増減なし)。
  ★1 度 `test_repo_hygiene`(CRLF 検査)で落ちた —— Python の text mode 書き込みが
  `logs/OPEN-ITEMS.md` を CRLF にしたため。**バイナリで LF に戻して再実行し全 PASS**
- **★やっていないこと**: **殻の実装をしていない**(未決 2 件が先)/ **`θ` の根拠を代筆していない**
  (規範的な線引き。`CLAUDE.md` §8)/ **`eval.cells` を凍結していない**(B1 の判定値はこれ待ち)/
  **実験を回していない** / **ポッドを起動していない**
- **影響を受けたファイル**: `logs/DECISIONS.md` / `plans/PLAN-001-eval-battery.md` /
  `configs/exp_phase1_main.yaml` / `plans/PLAN-020-theta-decision.md` / `logs/OPEN-ITEMS.md` /
  本ファイル / `STATE.md`
...
### docs(plan): PLAN-021。殻の測り方の決定材料(決定 0 件)+ 検査1 + ★F122 / ★F123 / ★F124   [actor: PLANNER / CRITIC]

- **`logs/HANDOFF.md`(その29)は 1 セッション古かった。**そこに書かれた唯一の作業(ADR-070 を書く)は
  **その30(commit `cf4a77d`)で完了しており、その30 は `logs/HANDOFF.md` を更新しないまま終わっていた。**
  `STATE.md` が正しい状態を持っていたのでそちらに従い、**順5 の残りブロッカーに着手した**
- **`plans/PLAN-021-shell-measurement.md` を新設した。決定は 0 件である。**
  上流は **ADR-070 決定4(θ-c = C2)**。C2 は `M*` の判定を累積平均から殻の正答率に変えたが、
  **殻をどう切りどこから引くかを決めていない**(ADR-070 自身が「本 ADR が決めていないもの」に挙げている)
- **検査1**(`plans/PLAN-021-check1/shell_capacity_check.py`。**組合せ論の計数であって実験結果ではない**)。
  殻の定義 3 通り × 13 格子点について、殻の組数・経路 (a) で殻に落ちる期待本数・二項 SE・
  経路 (b) の成否を出した。**SE はすべて「もし真の正答率が 0.70 だったら」という条件つきの数**であり、
  **素のモデルの正答率は 1 度も測っていない**

**★このセッションで新しく分かったこと(3 件。うち 2 件は `logs/OPEN-ITEMS.md` の現行の書き方を訂正する)**

- **★F122: 経路 (a) の「追加 GPU 時間 0」は経路 (b) に対する優位になっていない。**
  **順5 は 1 度も回っていない**(`results/` は空)ので、**(b) も同じ 13,000 項目の 1 回の run で済み、
  GPU 時間は (a) と同じ**である(承認済 1.5 GPU時間の中)。
  **さらに (a) 自身が ADR-041 決定5 の変更である** —— 決定5 は
  **「n は M 間で同一」**を明文で要求している(`configs/exp_phase1_main.yaml:526`)が、
  **(a) の n は 19.8 件(`M=100`)から 1,000 件(`M=25`)まで 51 倍動く**。
  **(a) と (b) の差は GPU ではなく、決定5 のどの条項を変えるかである。**
  (a) が実際に持つ利点は「累積表がそのまま残る」ことだけで、**案 (c)(両方引く)はそれを GPU 約 2 倍で買う**
- **★F123: 定義 A も定義 B も、外挿腕が実際に使う組をほとんど含まない。**
  主軸 3 水準目 `extrap_magnitude` は **`a > 99` かつ `b > 99`**(★F120。`code/data_gen/pool.py:195`)。
  殻は `|a| > 99` **または** `|b| > 99` なので、**負の被演算子と片側だけ域外の組**を大量に含む。
...
- **★F124: 定義 B は台地アンカー 4 点(25 / 50 / 75 / 99)で殻が空になる。**
  ADR-041 決定3 規則2 は「**小さい順に見て**初めて θ を割った水準の 1 つ下」を採るので、
  **空の水準の扱いを決めないと規則2 が回らない。**
  → **「殻-c(空・極小の水準の扱い)」を新しい人間待ちとして `logs/OPEN-ITEMS.md` に開いた**

**★補足(定義では動かないもの)**: `M = 100` の殻は**定義 A でも B でも 800 組**である
(1 つ前の格子点が 99 = 主域の半径だから)。**選択の余地がない。**経路 (a) だと n = 19.8、SE = 0.103 で、
これは 13 水準のうち最も粗い。**外挿腕が生き残るには `M = 150` までの全格子点が θ を超える必要がある**
ので、`M = 100` はその判定経路の上にある。

- **回帰テスト 6 件**を `code/tests/test_design_facts.py` に追加した(★F122 / ★F123 / ★F124 と
  「`M=100` の殻は定義によらず 800 組」)。**`pytest code/tests -q` = 906 → 912 passed**
- **★やっていないこと(意図的)**: **殻を実装していない**(未決 3 件が先。手を付けると人間の決定を先取りする)/
  **`θ = 0.70` の根拠を代筆していない**(規範的な線引き。`CLAUDE.md` §8)/
  **`eval.cells` を凍結していない** / **`configs/` を触っていない**(`shell_definition` / `shell_n_items` は
  `null` のまま。決まるまで既定値を作らない)/ **実験を回していない** / **ポッドを起動していない**
- **影響を受けたファイル**: `plans/PLAN-021-shell-measurement.md`(新設)/
  `plans/PLAN-021-check1/{shell_capacity_check.py,README.md,run.log}`(新設)/
  `code/tests/test_design_facts.py` / `logs/OPEN-ITEMS.md` / `logs/STATE-ARCHIVE.md` /
  `STATE.md` / `logs/HANDOFF.md` / 本ファイル

### docs(adr): ADR-071 —— 人間がエージェントの推奨案を一括採択した(D / (c) 限定版 / c3)   [actor: PLANNER / CRITIC]

- **人間の指示**: 「どれも AI の推奨案を採用する。推奨案がない場合は、全体を俯瞰的にみて推奨案を作製して」。
  **`plans/PLAN-021` §4 は推奨を付けずに案を並べていた**ので、**推奨案そのものを ADR-071 で新たに作り、
  人間が一括で採択した**。**したがって選択肢の比較検討はエージェントが行っている。**
  **ADR-071 の冒頭にこの性質を明記した**(`CLAUDE.md` §8 / ADR-039 決定3。**隠さない**)
- **決定1(殻-a = 案 D)**: **判定は定義 C = `Q(M)`**(`99 < a <= M` かつ `99 < b <= M` =
  `extrap_magnitude` の母集団)。**報告は現行の一様抽出(累積)と定義 A を併記する。**
  根拠は **★F123** —— 定義 A / B の殻は `M=150` で **94.9%** が C6 に入らない組である
- **決定2(殻-b = 案 (c) の限定版)**: **現行の 13 水準 × 200 × 5 = 13,000 項目を 1 文字も変えず**、
  **`|Q(M)| >= 200` を満たす 7 水準(`{125,150,175,200,300,500,999}`)に `Q(M)` から
  200 × 5 = 7,000 項目を足す**。合計 20,000 項目。根拠は **★F122**(「追加 GPU 0」は優位でない)+
  経路 (a) は決定5 の「n は M 間で同一」を破る + **足す形なら ADR-041 決定5 に打ち消し線が要らない** +
  `Q(150)` の二項 SE が **0.086 → 0.014**(**仮定正答率 0.70 の下**)になる
- **決定3(殻-c = 案 c3)**: **判定水準は「凍結済の 200 件を `Q(M)` から引ける水準」**に限る。
  **閾値に新しい数を作らない**(200 は ADR-041 決定5 の凍結値)。
  `M ∈ {25,50,75,99,100,110}` は一様抽出の腕で測り**記述として報告する** ——
  `M* <= 110` は ★F120 + ADR-070 決定3(B1)で外挿腕が取り下げられるので**結論を変えない**
- **★帰結**: **外挿腕が生き残るのは `Q(125)` と `Q(150)` の正答率がともに `θ = 0.70` 以上のときだけ**
  である(`Q(150)` が割れば `M* = 125 < 134` で B1 が発火する)。
  **格子と規則の帰結であって実測ではない**
- **★GPU 見積りが変わった**: `estimated_gpu_hours` **1.5 → 2.5**(打ち消し線 + 理由 + 日付を config に書いた)。
  **`CLAUDE.md` §2 の 10 GPU時間の門は超えない**が、**2026-09-06 の承認は 1.5 時間の見積りに対して
  出ている**ので記録に残した
- **★テストが実装より先に罠を 1 つ潰した**: 判定水準の導出を `(M-99)^2 >= 200` と書くと、
  **`M < 99` でも平方が正になり台地アンカーが判定水準に混じる**(`Q(M)` は `M <= 99` で 0)。
  最初に書いたテストがこれで落ちた。**config のコメントにも同じ式を書いていたので直した**
- **反映**: `logs/DECISIONS.md`(ADR-071)/ `configs/exp_phase1_main.yaml`
  (`shell_definition` = `"extrap_magnitude_quadrant"` / `shell_n_items` = 200 /
  `shell_radii` / `shell_judgement_radii` / `estimated_gpu_hours` 2.5)/ `plans/PLAN-021` §6 /
  `logs/OPEN-ITEMS.md`(**3 行に打ち消し線**)/ `STATE.md` / `logs/STATE-ARCHIVE.md`
- **テスト**: `pytest code/tests -q` = **912 → 914 passed**(判定水準の導出と「生死は 2 水準で決まる」を固定)。
  ★**1 度だけ `test_power_sim.py::test_run_fits_writes_one_manifest_per_shard` が落ちた。**
  **単独再実行と全体の再実行では通る**(914 passed)。**並列実行の絡む fixture の flaky が疑わしい。
  再現していないので原因は未特定である。次に落ちたら追うこと**(`CLAUDE.md` §7)
- **★やっていないこと(意図的)**: **殻を実装していない**(次セッション。`code/eval/battery/magnitude_sweep.py` /
...
### docs(plan): 順5 の掃引結果を報告した(4 値分解 + §7 の点検)+ ★F126 を人間に上げた   [actor: RUNNER]

- `runs/20260910_104249_sweep_m/metrics.json` [run:20260910_104249_sweep_m] の `quadrant`(判定の材料)/ `by_radius` / `grid_shell`(記述)を読み、
  **4 値分解・シード間 SD・`timing` を `STATE.md`「わかっていること」の先頭に run_id とセットで載せた。**4 値の合計は全ブロック・全水準・全シードで 1.0
- **`CLAUDE.md` §7 の点検**(`predictions/` は件数だけ数えた。全文は読んでいない):
  - `parse_fail` の大半は **PLAN-001 §5.4.1 規則2(整数がちょうど 1 個)が同じ答えの言い直しを落としたもの**(仕様どおり)。
    本番パーサの関数で数えた診断: 腕2 の 160 件中 133 / 腕1 の 2,258 件中 1,264 が「最後の印の後ろの整数がすべて真値と同じ値」。`p2` と同じ値は 0 件
  - 腕1 の低下は負の被演算子に集中(両方 ≥ 0 は correct 0.982 / 両方 < 0 は 0.641)。素のモデルで `rule` 26 件(腕1 のみ・負の被演算子を含む項目のみ)
  - M=125 は母集団 676 から 1,000 回引くのでシード間で組が重なる(異なる組 563)
  - `dirty: true` / `git_diff.patch` 0 バイトは `code/artifacts.py` の取り方どおり(追跡ファイルの変更なし)
- `logs/OPEN-ITEMS.md` に **★F126**(`M*` の前にパーサ規則2 を見直すか。案 (i)〜(iii))を登録 / 停止中ポッドの行に `omjvbdanmbrzc8` を追加
...
### docs(plan): PLAN-024 —— 順6 の結果を人間が読むための判断材料(D1〜D7)を並べた   [actor: PLANNER (Opus)]

- HANDOFF(その44)の 1 件だけを行った: `logs/OPEN-ITEMS.md` の「PLAN-023 §6.1 の実装の読み」「順6 の結果の読み」「★F138」を、選択肢と「どの文書が変わるか」の形に並べた。
  **決定は 0 件・推奨は付けていない**(`CLAUDE.md` §8。HANDOFF の禁止事項)。GPU 0・ポッドは触っていない
- 読み取りだけの点検(R1〜R5 の `predictions/` と `compare_vs_r1.json`。書き換えていない):
  - T3・T1b のセル別・極性別の Yes 件数(T1b は gt と lt で Yes の率がほぼ同じ。T3 は lt で 0/240)。プロンプトの文面が極性どおりであることを確認(テンプレートの取り違えは無い)
  - **★F139(新)**: 最初の位置で Yes/No 12 綴りに乗る確率の合計が T1b で中央値 0.0009 / 0.0004、T3 で 0.98 / 0.99
  - **★F140(新)**: T1 の parse_fail 2 件は `The sum of a and b is N.`(N は真値)で終わる応答。`numeric.parse` が `None` を返すことを手元で確認
  - R4 の強制選択 13 件は両方の run で |差| ≤ 0.25(bf16 の刻みと整合。原因とは確かめていない)。R2・R3 の強制選択の対数尤度は R1 と 960/960 で文字列一致
  - `gonogo` を R4 で組んでも印の位置は R1 と同じ(ファイルには書いていない)
  - 3 行の対象外で持ち越す事実: T2 の 5 場面の `correct_rate` の分散は 0(Δ の移し替え元)/ `spec_mul` の other_error .233
- `logs/OPEN-ITEMS.md`: 3 行の「正本」欄に PLAN-024 への参照を足し、★F139・★F140 を新設
- `STATE.md`: 冒頭・いま何をしているか・わかっていること ★順6(3 行追加)・ブロッカー 2・次のアクション・引き継ぎを差し替え(392 行)。旧ブロックは `logs/STATE-ARCHIVE.md`「その45」へ移した(`git show 0c57599:STATE.md` から機械的に切り出し)

## 2026-09-12(その46)

### docs(plan): PLAN-024 に §1.8(推奨の根拠の数え上げ)と §4.1(エージェントの推奨)を足した   [actor: PLANNER (Opus)]

- **人間の依頼**(「PLAN-024 §4 について何をどのように決定すればいいか説明し、推奨案が作成できる部分に関しては推奨案も作成してください」)で、
  その45 の「推奨は付けない」を解いた。**推奨は案であって決定ではない**(`CLAUDE.md` §8)。§4 の記入欄は空のまま。GPU 0・ポッドは触っていない
- 読み取りだけの数え上げ(§1.8。run_id 付き):
  - ★F140 の形(最終行 `(sum|result|answer|total) … is N.`)は、順5 の再採点後に残った parse_fail のうち **腕2 で 22 件中 21 件(N = 真値)**、腕1 で 902 件中 462(真値)+ 64(非真値)[run:20260910_215422_rescore_sweep_m]。
    いま抽出できている応答でこの形と抽出値が食い違うものは 0 件。**正規表現による数え上げであって、パーサを変えたときの遷移数ではない**
  - R4 対 R1 で分類が変わった 16 件は二値群 13・`spec_mul` 3 で、**T1・指示付き T1・T2 は 0 件**(応答文字列の差は 5 / 13 / 50 件あった)[run:20260911_161738_order6_r4]
  - 二項分布の算術 2 つ(#2 の最良二値セルが偶然で割る確率 ≈ 1.1e-6 / #1 をセル n = 80 で読むと真の率 0.01 でも約 19% で割れる)
- 推奨(§4.1): D3 (a)(+ 任意で上位 k の器械)/ **D4 (a)**(D4 を動かしうるのは D3 (c) だけ、という依存の整理つき)/ D2 (b)(R8 の事前確認は Phase 1 の GPU 構成と別に諮る)/ D7 (a)/
  D5 (b)(D4 で二値群を残すなら (c))/ D1 (a)/ **D1-F140 (b)** / D6 は人間。**推奨をすべて採れば GPU は要らない**

### docs(plan): PLAN-024 §3.0 に「各選択が何を・どのように決めるか」を足し、セッション引き継ぎを書いた   [actor: PLANNER (Opus)]

- 人間の依頼(「実験の結果をもとにそれぞれの選択が何をどのように決める選択であるか説明してほしい」)に答え、同じ内容を PLAN-024 §3.0 の表にした。決定は 0 件
- context-guard(約 18.6 万トークン)が出たので skill `handoff` で切った。`STATE.md` の 4 ブロック(冒頭・いま何をしているか・次のアクション・引き継ぎ)を差し替え、旧ブロックは `logs/STATE-ARCHIVE.md`「その46」へ機械的に移した。「わかっていること ★順6」に §1.8 の 1 行を足した
- 差し替えに使った Python の `write_text` が Windows で CRLF を書き、`test_repo_hygiene.py` が 2 件落ちた → LF に戻して 7 passed

## 2026-09-12(その47)

### docs(plan): 二値選択の結果を人間に平易に説明し、根拠の数え上げを PLAN-024 §1.9 に残した + 引き継ぎ   [actor: PLANNER (Opus)]

- 人間の依頼(「二値選択の実験の結果をわかりやすい言葉で説明し、その理由や解釈の分析も行ってください」)にチャットで答えた。理由・解釈は候補として出し、決定は 0 件(`CLAUDE.md` §8)。GPU 0・ポッドは触っていない
