...
> **★2026-09-09(その28)に `STATE.md` から移した**(ADR-063 運用規約1。**1 文字も削っていない**)。

> **★2026-09-09(その27・最新)。Phase 0。PLANNER / CRITIC。GPU 時間 0。**
> **★★人間が 4 束すべてを決め(ADR-069)、エージェントが 4 件とも実装まで終えた。**
> **★F112 = 案 (a)** —— `configs/power_sim*.yaml` に **`effect.eta`(丸める前の 12 セル)**を
> 新設して DGP の真値にした。**`effect.rule_rate` は「`eta` を丸めたものと一致するか」の門**として
> 残る(離れたら止まる)。**非加法性 RMS が 0.3605 → 0.352767**(§6.5 の 0.353 と 3 桁で一致)、
> **★P0 は厳密に 0 になり、`sigma = 0` の行が α の較正として読める。★F112 は閉じた。**
> **★F113 = 案 (a)** —— `Documents/05_STATISTICS.md` **§5 に「★順6 からの移し替え」を新設**し、
> **食い違い 3 つと移し替えの 3 仮定を明示した**(旧記述は打ち消し線 + 理由 + 日付で残る)。
> **★3 仮定は検証されないまま凍結に入る**(論文の弱点欄)。**順6 の GPU は依然として要る。**
> **★F114 = 並列化だけ先に実装** —— `shard_jobs` / `run_fits` / `fit.n_workers` / `--workers`。
> **R の起動は 1 並列単位につき 1 回のまま。★smoke を 1 並列と 4 並列で回し、
> summary が `n_workers` 以外 完全に一致することを実測で確認した**(結果は並列数に依らない)。
> **`describe_plan` のコスト行を実測に差し替えた: 992 コア時 / 16 並列で 62 時間。**
> **★実行先(この機械 / RunPod)は決めていない**(`logs/OPEN-ITEMS.md` へ送った)。
> **★実装判断 4 件は全件承認された。★(2) の穴は未実装の `primary.py` にも同じ形で効く。**
> **`pytest code/tests -q` = 884 → 898 passed。**
> **実験は 1 件も実行していない。`results/` は空。凍結タグは打っていない。**

> **これより前のブロックは `logs/STATE-ARCHIVE.md` §4 にある。**

> **★2026-09-09(その27)に `STATE.md` から移した**(ADR-063 運用規約1。**1 文字も削っていない**)。
...
> **★2026-09-09(その26)に STATE.md から移した(ADR-063 の運用規約1)。その25 のブロック。**

> **★2026-09-09(その25・最新)。Phase 0。IMPLEMENTER (Opus)。GPU 時間 0。**
> **★★★★F90-2 の実装が終わった。**ADR-067(案 B)と ADR-068(★F103 の 3 件)を
> **`Documents/05_STATISTICS.md`(事前登録の本体)と `code/` に反映した。**
> **§6.3 手続き2〜5 / §6.4 / §6.6 を差し替え(旧記述は打ち消し線 + 理由 + 日付)、
> ★§6.7「掃く仮定値の範囲」を新設した**(見出しに「★すべて仮定値」と明示)。
> **`code/analysis/power_sim.py` + `power_sim_fit.R` + `configs/power_sim*.yaml` を新規実装。**
> **DGP の交換可能な `Sigma` + §3.2 の当てはめ対 + ADR-059 の縮退カスケード(判定はペア単位)。**
> **`pytest code/tests -q` = 839 → 874 passed。**
> **★本実行は 1 度も回していない** —— `dgp.n_item`(**`M*` = 順5 待ち**)と
> `dgp.s2_item` / `dgp.s2_tmpl`(**★F104**)が `null` で止まるためである
> (**エージェントは既定値を作っていない**。skill `code-style` §5)。
> **`--dry-run` は通り、当てはめ本数 24,000 本を出す。**
> **★新しく開いたのは ★F112(§6.5 の表の丸めと RMS)と実装判断 4 件だけである**
> (`logs/OPEN-ITEMS.md` の「その25」ブロック / `plans/PLAN-019` §10.10)。
> **実験は 1 件も実行していない。`results/` は空。凍結タグは打っていない。**
...
> **★★2026-09-09(その27・最新)。ADR-069 で 4 束が片付いた。残りは下の 6 本である。**
>
> 1. **★F104 を人間が確認する**(`plans/PLAN-019` §10.4)——
>    **`s2_item` / `s2_tmpl` の取得元。**順6 の DV は `correct_rate`、シミュレータは `is_rule`。
>    **`M*` と合わせて、`power_sim.py` の本実行を止めている 2 本である**
> 2. **★`θ` を決める**(ADR-041 決定2・決定3)。**順5 の唯一の残りブロッカー。**
>    **2026-09-07 以降、どのセッションでも動いていない。★`M*` は順5 の出力なので 1 に効く**
> 3. **★順6 を回す GPU を承認する。**Go/No-Go #0〜#3 はここでしか出ず、**段階 D の前に要る**。
>    **★F113 が決着した今も、順6 が要ることは変わらない**(ADR-069 決定2 の「倒れないもの」)
> 4. **★F114 の実行先を決める**(`logs/OPEN-ITEMS.md`)。**並列化は済んでいる。**
>    **1 と `M*` が決まってから諮るのが自然である**(どのみち本実行はそこで止まる)
> 5. **`code/analysis/primary.py`**(PLAN-016 §7-5)。**N5 が残っている。**
>    **★ADR-069 決定4 (2) は primary.py にも同じ形で効く**(実装時に適用する)
> 6. **`Documents/09_PAPER_PLAN.md` / `00_OVERVIEW.md:7` が再設計前のままである**(人間待ち)
...
> **★★2026-09-09(その25・最新)。★F90-2 の実装は終わった。★F103 も閉じている。**
>
> 1. **★F112 を人間が確認する**(**新規**。`plans/PLAN-019` §10.10.1)——
>    **§6.5 の表(小数第2位)からは、同じ節が載せる非加法性 RMS が復元しない**
>    (P1 は 0.353 に対し 0.3605、**★P0「完全に平行」= 0.000 は 0.0285**)。
>    **丸める前の 12 セルを config に入れるか、`sigma = 0` 行を α の較正として読まないか**
> 2. **★実装判断 4 件の採否**(`logs/OPEN-ITEMS.md` の「その25」)——
>    **シード掃きを 10 に畳んだ**(残すと 24,000 → 96,000 本)/
>    **ADR-059 の穴(当て直した場合どちらを LRT に入れるか)を `refit` にした** ほか
> 3. **★F104 を人間が確認する**(`s2_item` / `s2_tmpl` の取得元)。
>    **これが決まらないと `power_sim.py` の本実行が回せない**(`M*` と合わせて 2 本)
...
>    **★ADR-059 の穴(上の 2)は primary.py にも同じ形で効く**
...
> **★★2026-09-09(その27・最新)。PLANNER / CRITIC (Opus)。**
>
> **★やったこと**: **人間に 4 束を諮り、4 束とも決定を得て(ADR-069)、4 件とも実装まで終えた。**
> **決定 = ★F112 案 (a) / ★F113 案 (a) / ★F114 は並列化のみ / 実装判断 4 件は全件承認。**
>
> **★書き換えた先**: **`logs/DECISIONS.md` の ADR-069** / **`configs/power_sim.yaml` +
> `power_sim_smoke.yaml`**(`effect.eta` / `fit.n_workers`)/ **`code/analysis/power_sim.py`**
> (`inv_logit` / `_require_shape` / `shard_jobs` / `run_fits` / `SECONDS_PER_PAIR_N_ITEM_48` /
> `--workers`)/ **`Documents/05_STATISTICS.md` §5**(「★順6 からの移し替え」を新設)/
> **`code/tests/test_power_sim.py`**(884 → **898 passed**)/ **`logs/OPEN-ITEMS.md`**。
>
> **★確かめたこと(実測)**: **smoke を `--workers 1` と `--workers 4` で回し、
> `power_sim_summary.json` が `n_workers` 以外 完全に一致した** —— **結果は並列数に依らない。**
> **`build_profile` の RMS が 0.352767**(§6.5 の 0.353 と 3 桁で一致)、**P0 は厳密に 0。**
>
> **★やっていないこと(意図的)**: **本実行を回していない**(`n_item` / `s2_item` / `s2_tmpl` が null)/
> **★F114 の実行先を決めていない**(人間へ送った)/ **`n_item = 96` のベンチを完走していない** /
> **文献を 1 件も探していない** / **凍結タグを打っていない** / **GPU を起動していない**。
>
> **★次セッションが引き継ぐもの**: **★F104** / **`θ`** / **順6 の GPU** / **★F114 の実行先** /
> **N5 と `code/analysis/primary.py`** / **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`** /
...
> **★★2026-09-09(その25・最新)。IMPLEMENTER (Opus)。**
>
> **★やったこと**: **★F90-2 の実装。**ADR-067 + ADR-068 を `Documents/` と `code/` に反映した。
> **人間の決定は 1 件も求めずに始め、1 件も蒸し返していない。**
>
> **★書き換えた先**:
> - **`Documents/05_STATISTICS.md`**: §6 冒頭のファイル一覧 / **§6.3 手続き2〜5**
>   (**手続き3 を DGP・当てはめ・集計の 3 行に分けた** —— ★F110)/ **§6.4**(節題ごと。
>   ★F102 の用語訂正を含む)/ **§6.6 末尾** / **★§6.7 を新設**
> - **`code/analysis/power_sim.py`**(新規)/ **`power_sim_fit.R`**(新規)/
>   **`configs/power_sim.yaml` / `power_sim_smoke.yaml`**(新規)/
>   **`code/tests/test_power_sim.py`**(新規 35 件)
> - **`plans/PLAN-004` §3 順7 と §6 罠4**(順6 依存が外れた)/
>   **`plans/PLAN-019` §10.10 を新設**(★F112 + 実装判断 4 件 + 限界)/ **`logs/OPEN-ITEMS.md`**
>
> **★確かめたこと**: **R 4.6.1 / lme4 2.0.6 で smoke config が端から端まで通る** ——
> **`df = 6` が返り、ADR-059 の当て直しが実際に発火した**(`add_loglik_gain` / `add_beta_move` が出た)。
> **★そこから出た数値は主張に使えない**(4 シード / 2 項目 / 2 反復)。
>
> **★やっていないこと(意図的)**: **本実行を 1 度も回していない**(null で止まる)/
> **縮退の発火率を測っていない**(ADR-068 決定1 の利②はまだ数値になっていない)/
> **当てはめ 1 本の時間をこの機械で測っていない**(コストは依然 F50 からの掛け算)/
> **文献を 1 件も探していない** / **`05_STATISTICS.md` §5 の「Phase 0(順6)」は触っていない**
> (ADR-067 / ADR-068 の影響欄が §6 に限っているため。**同じ表記の穴が §5 に 1 箇所残る**)/
> **凍結タグを打っていない**。
>
> **★次セッションが引き継ぐもの**: **★F112 と実装判断 4 件の採否**(人間)/ **★F104** /
> **`θ`** / **順6 の GPU** / **N5** / **`code/analysis/primary.py`**。
...
## ★2026-09-09(その29)にアーカイブへ移したブロック(ADR-063 運用規約1)

### 旧「最終更新」(その28)

最終更新: 2026-09-09(その28)/ by PLANNER / CRITIC (Opus)
(**★F104 の決定材料を作った(決定 0 件)。記入欄は `plans/PLAN-019` §10.13.5、要約は §10.13.0。**
**★F115** = 順6 の形では `s2_item` の同定が効かない(★F90 と同じ形の穴)/ **★F117** = 交互作用へ
直接入るのは `s2_item` だけで、シード数では縮まない(縮むのは `n_item` = `M*` 待ちのみ)。
**検査3(合成データと算術)+ 回帰テスト 2 件。`pytest` = 898 → 900 passed。**
**事前登録の本体と入力は触っていない。実験は 1 件も実行していない。`results/` は空。GPU 時間 0。**)


### 旧「いま何をしているか」(その28)

> **★2026-09-09(その28・最新)。Phase 0。PLANNER / CRITIC。GPU 時間 0。**
> **★F104 の決定材料を作った(決定 0 件)。****問いの形が変わり、決定が 4 行に割れた。**
> **記入欄は `plans/PLAN-019` §10.13.5、1 ページ要約は §10.13.0 である。**
> **★F115 —— 順6 の形では `s2_item` の同定が効かない。**順6 は 1 本の run(素のモデル)で、
> T2 以外の項目は **1 回ずつ**しか観測されない。**1 項目 1 観測の Bernoulli では
> profile 対数尤度が完全に平らである**(検査3 [A1])。**タスク5 の反復 3 回は温度 0 であり、
> 一致する限り情報を持たない**(検査3 [A2])。**★「移し替えてよいか」以前に元の量が立つ保証が無い。**
> **★F90 と同じ形の穴である。**
> **★F117 —— `task:coverage` の交互作用へ直接入るのは `s2_item` だけである。**
> `n_item = 48` で `s2_item` = 0.5 / 1.0 / 2.0 / 4.0 のとき非加法性 RMS が
> **0.098 / 0.139 / 0.195 / 0.276**(採択済み P1 = 0.352767 の **28% / 39% / 55% / 78%**)。
> **`s2_tmpl` も対照の `sigma`(seed)も この水路では厳密に 0。**
> **★`s2_item` はシード数では縮まない**(`v[item]` は 10 シード共有)。**縮むのは `n_item` だけで、
> それは `M*` 待ちである。****★したがって 2 つを 1 つの決定にする理由が無い。**
> **★F116 / ★F118 / ★F119** —— 順6 の `s2_tmpl` は T2 の 5 水準のみ(当てはめは 10 水準)/
> **`s2_tmpl` に 0 に近い値を置くと縮退カスケードの第1段が常時発火する**(★未測定)/
> **順6 のテンプレート間分散 1 個は ADR-069 決定2 が既に Δ の床へ割り当てている。**
> **★検査3 を足した**(`plans/PLAN-019-check3/`。**合成データと算術。実験結果ではない**)。
> **回帰テスト 2 件で ★F117 が乗る `draw_frame` の構造を固定した**(★変異させて落ちることも確認)。
> **`pytest code/tests -q` = 898 → 900 passed。**
> **★`Documents/05_STATISTICS.md` と `configs/power_sim.yaml` は触っていない**(人間の決定待ち)。
> **実験は 1 件も実行していない。`results/` は空。凍結タグは打っていない。**

### 旧「引き継ぎ」(その28)

> **★★2026-09-09(その28・最新)。PLANNER / CRITIC (Opus)。**
>
> **★やったこと**: **★F104 の決定材料を作った**(`logs/HANDOFF.md` その27 の指示どおり。**決定 0 件**)。
> **★F104 は「事実はあるが案が無い」状態だったので、まず測ってから案を並べた。**
> **ADR-067 / ADR-068 / ADR-069 は 1 文字も触っていない**(蒸し返し禁止の指示を守った)。
>
> **★書き換えた先**: **`plans/PLAN-019-validity-decisions.md` §10.13 を新設**(§10.13.0 要約 /
> §10.13.1 の F115〜F119 / §10.13.2 の `s2_item` 5 案 / §10.13.3 の `s2_tmpl` 3 案 /
> §10.13.4 / **§10.13.5 記入欄(空)** / §10.13.6 限界)。§10.8 の F104 行と §10 冒頭注も追随。
> **`plans/PLAN-019-check3/`**(README / スクリプト / `run.log`)/ **`code/tests/test_power_sim.py`**
> (**898 → 900 passed**)/ **`logs/OPEN-ITEMS.md`** / **`logs/CHANGELOG.md`**。
>
> **★確かめたこと(合成データと算術。実験ではない)**: **1 項目 1 観測では `s2_item` の
> profile 対数尤度が完全に平ら**(差 0、最大でも `2.8e-14`)/ **温度 0 の反復が一致する限り
> 対数尤度は単調に増える**(内点の MLE が無い)/ **交互作用へ直接入るのは `s2_item` だけで、
> `s2_tmpl` も `sigma` も厳密に 0**(対照行つき)/ **`s2_item` の寄与は `1/sqrt(n_item)`**。
> **★回帰テストを変異させて落ちることを確認した**(seed ループ内で `v` を引く形にすると落ちる)。
>
> **★やっていないこと(意図的)**: **`glmer` を 1 本も当てていない**(**検出力そのものは
> 1 つも出していない**)/ **事前登録の本体と入力を触っていない**(`05_STATISTICS.md` /
> `configs/power_sim.yaml`。人間の決定が要る)/ **`s2_item` / `s2_tmpl` の値を提案していない** /
> **本実行を回していない** / **文献を 1 件も探していない** / **GPU を起動していない**。
>
> **★次セッションが引き継ぐもの**: **★F104 の決定**(§10.13.5)/ **`θ`** / **順6 の GPU** /
> **★F114 の実行先**(★F104-a に従属)/ **N5 と `code/analysis/primary.py`** /
> **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`** / **★L(d)** / **★C** / **★E** / **E-5 (b)** /
> **★2** / **PLAN-018 §4.3**。


---

## ★2026-09-09(その29)—— STATE.md から移した 5 ブロック(ADR-063 運用規約1)

> **その30(ADR-070 の記録)で置き換えたブロックである。1 文字も削っていない。**

### ヘッダ(その29)

最終更新: 2026-09-09(その29)/ by PLANNER / CRITIC (Opus)
(**★`θ` の決定材料を作った(決定 0 件)。記入欄は `plans/PLAN-020` §7、要約は §0。**
**★F120** = 主軸 3 水準目 `extrap_magnitude` の母集団は `(M* − 99)²` しかなく、`M* = 100` で **1 組**。
セル表の要求 520 組(carry 層 240)を満たす最小の `M*` は **134**、格子上は **150**。
**ADR-041 決定3 規則5 は `M* < 100` にしか分岐が無く、格子点 100 / 110 / 125 はその外側にある** /
**★F121** = `correct_rate(M)` は殻ではなく `R(M)` 全体の累積平均で、`M = 100` では引かれる組の
**98.0%** が主域の中にある。**仮定シナリオでは懸念6 が外挿腕に戻る。**
**検査1(組合せ論の計数)+ 検査2(仮定を置いた算術)+ 回帰テスト 6 件。`pytest` = 900 → 906 passed。**
**事前登録の本体と `configs/` は触っていない。実験は 1 件も実行していない。`results/` は空。GPU 時間 0。**)

### いま何をしているか(その29)


> **★2026-09-09(その29・最新)。Phase 0。PLANNER / CRITIC。GPU 時間 0。**
> **★`θ` の決定材料を作った(決定 0 件)。****問いの形が変わり、決定が 4 行に割れた。**
> **記入欄は `plans/PLAN-020-theta-decision.md` §7、1 ページ要約は §0 である。**
> **θ-0**(いつ決めるか)/ **θ-a**(値)/ **θ-b**(`M*` が 100〜133 の分岐)/ **θ-c**(殻の正答率)。
> **★F120 —— 主軸 3 水準目の母集団は `(M* − 99)²` しかない。**
> `MAIN_COVERAGE_LEVELS` は `id` / `interp` / **`extrap_magnitude`**(`code/data_gen/pool.py:59`)で、
> `extrap_magnitude` は **`a > 99` かつ `b > 99`**(両側正)。**`M* = 100` で 1 組 / 110 で 121 組 /
> 125 で 676 組。**セル表の要求は **520 組(carry 層ごとに 240 組)**。
> **C6 と carry 層をともに満たす最小の `M*` は 134、掃引格子上の最小の成立点は 150。**
> **★ADR-041 決定3 規則5 は `M* < 100`(= `D_ext` が空)にしか分岐が無く、
> 格子点 100 / 110 / 125 はその外側にある。**規則どおり進むと `fill_cells` が
> **順5 の後・凍結の直前に**例外で止まる。
> **★F121 —— `correct_rate(M)` は殻ではなく `R(M)` 全体の累積平均である。**
> `magnitude_sweep.build_items` は `[-M, M]²` から一様に引くので、**`M = 100` では引かれる組の
> 98.0% が主域 `R(99)` の中にある。****仮定シナリオ S1(崖が 99)では θ = 0.50 〜 0.90 の
> どれを選んでも `M*` が 100 / 110 / 125 に落ち、外挿セルは素のモデルの正答率 0 の項目で埋まる** ——
> **`plans/PLAN-001` §4.1.1 が「設計段階で潰れる」と書いた懸念6 そのものである。**
> **★殻の正答率は追加の GPU 時間 0 で取れる**(`predictions/` に `operands` が残る。`run.py:681`)。
> **★検査を 2 本足した**(`plans/PLAN-020-check1/`。**組合せ論の計数と、仮定を置いた算術**)。
> **回帰テスト 6 件**(`test_design_facts.py`)。**`pytest code/tests -q` = 900 → 906 passed。**
> **★`Documents/` と `configs/` は 1 文字も触っていない**(人間の決定待ち)。
> **実験は 1 件も実行していない。`results/` は空。凍結タグは打っていない。**

### ブロッカー1(その29)

1. **`θ`(外挿域の閾値)が未決である**(ADR-041 決定2・決定3)。**順5 の唯一の残りブロッカー。**
   **★2026-09-09(その29)に決定材料を作った**(`plans/PLAN-020` §7 が記入欄。**決定 0 件**)。
   **★決定は 4 行に割れた**(θ-0 / θ-a / θ-b / θ-c)。**★θ-b(`M*` が 100〜133 のときの分岐)は
   規則に存在しない** —— 規則5 は `M* < 100` にしか分岐が無く、**格子点 100 / 110 / 125 では
   `D_ext` が空でないのに主軸 C6(520 組・carry 層 240 組)が組めない**(★F120)。
   **★θ-0(いつ決めるか)自体が記録で食い違っている**(PLAN-020 §3)

### 次のアクション 2(その29)

> 2. **★`θ` を人間が決める**(`plans/PLAN-020` **§7**。記入欄は **4 行**)——
>    **θ-0**(順5 の前か後か)/ **θ-a**(値)/ **θ-b**(`M*` が 100〜133 の分岐。**★本命**)/
>    **θ-c**(殻の正答率を報告するか)。**順5 の唯一の残りブロッカー。**
>    **★エージェントは θ-a の値を推していない**(規範的な線引き。`CLAUDE.md` §8)

### 引き継ぎ(その29)


> **★★2026-09-09(その29・最新)。PLANNER / CRITIC (Opus)。**
>
> **★やったこと**: **★`θ` の決定材料を作った**(ユーザーの依頼。**決定 0 件**)。
> **★材料を作る過程で、凍結済みの規則に分岐の無い領域が 2 つ見つかった**(★F120 / ★F121)。
> **ADR-041 / ADR-057 は 1 文字も触っていない。**
>
> **★書き換えた先**: **`plans/PLAN-020-theta-decision.md` を新設**(§0 要約 / §3 の θ-0 /
> §4 検査1 / §5 検査2 / §6 案 / **§7 記入欄(空)** / §8 限界 5 件)。
> **`plans/PLAN-020-check1/`**(README / 2 スクリプト / 2 ログ)/ **`code/tests/test_design_facts.py`**
> (**900 → 906 passed**)/ **`logs/OPEN-ITEMS.md`** / **`logs/CHANGELOG.md`**。
>
> **★確かめたこと(組合せ論の計数と、仮定を置いた算術。実験ではない)**:
> **`extrap_magnitude` の母集団は `(M* − 99)²`**(`M* = 100` で 1 組)/
> **C6 520 組と carry 層 240 組を満たす最小の `M*` は 134**(格子上は 150)/
> **`p2d` 判別不能の除外は carry 層に 1 組も効かない**(既知の事実の独立な再確認)/
> **`M = 100` では掃引が引く組の 98.0% が主域の中にある**。
> **★本番の `label_main_coverage` / `carry_label` を呼んでいる**(定義を検査側で複製していない)。
>
> **★やっていないこと(意図的)**: **θ の値を提案していない**(規範的な線引き。`CLAUDE.md` §8)/
> **`Documents/` と `configs/` を触っていない** / **殻ごとの正答率を実装していない**(取れることを
> コードの読みで確かめただけ)/ **本実行を回していない** / **文献を 1 件も探していない** /
> **GPU を起動していない**。
>
> **★同セッション追記**: **`plans/PLAN-020` §6.5**(元々の問い「見た vs 見ていない」との距離)。
> **理想の核は `id` vs `interp` で、これは `θ` に一切依存しない。****θ-c C2 が最も忠実 / θ-b B1 が核を保つ / θ-b B2 と θ-c C3 は理想の否定 / θ-a はほぼ無関係。**
> **次セッション用のプロンプトは `logs/HANDOFF.md`**(次は ADR-070 を書く役)。
>
> **★次セッションが引き継ぐもの**: **★`θ` の決定 4 行**(`plans/PLAN-020` §7)/ **★F104**
> (`plans/PLAN-019` §10.13.5)/ **順6 の GPU** / **★F114 の実行先**(★F104-a に従属)/
> **N5 と `code/analysis/primary.py`** / **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`** /
> **★L(d)** / **★C** / **★E** / **E-5 (b)** / **★2** / **PLAN-018 §4.3**。
...
## ★2026-09-09(その31)—— STATE.md から移した 4 ブロック(ADR-063 運用規約1)

> **その30(ADR-070 を書いたセッション)のブロックである。1 文字も削っていない。**

### ヘッダ(最終更新。その30)

最終更新: 2026-09-09(その30)/ by PLANNER (Opus)
(**★人間が `θ` の 4 行を採択した。ADR-070 を書いた**(A2 / **0.70** / B1 / **C2**)。
**θ-c = C2 により `M*` の判定が「累積」から「殻の正答率」に変わった** —— 推定量の変更である。
**θ-b = B1 により、`D_ext` が空でなくても主軸 C6 が組めなければ外挿を取り下げる**(df 6 → 3)。
**★合わせ技で外挿腕の生存条件は「格子点 150 までの各殻で正答率 0.70 以上」になった。現行規則より厳しい。**
**★C2 は測定を一意に定めず、人間待ちが 3 件新しく開いた**(殻の定義 / 殻あたりの項目数と抽出経路 / `θ` の根拠)。
`pytest code/tests -q` = **906 passed**。**実験は 1 件も実行していない。`results/` は空。GPU 時間 0。**)

### 「いま何をしているか」(その30)

> **★2026-09-09(その30・最新)。Phase 0。PLANNER。GPU 時間 0。**
> **★人間が `plans/PLAN-020` §7 の 4 行すべてを採択した**(2026-09-09)。**ADR-070 に記録した**
> (提案 エージェント (Opus) / 採択 人間。ADR-039 決定3)。
> **θ-0 = A2**(順5 の前に `θ` を確定し、`θ ± 0.05` での `M*` を副次として事前登録)/
> **θ-a = 0.70** / **θ-b = B1**(C6 が組めないなら外挿を取り下げ df 6 → 3)/
> **θ-c = C2**(**`M*` の判定そのものを殻の正答率に変える**)。
> **★C2 は `plans/PLAN-001` §4.1.1 の推定量の変更である**(設計変更。`CLAUDE.md` §4)。
> ADR-041 決定3 の**規則2・3・4 が指す `correct_rate` の中身が変わった**(規則の形は変えない)。
> **★4 決定の合わせ技で外挿腕の生存条件が厳しくなった** —— B1 + C2 の下では、外挿腕は
> **「格子点 150 までの各殻で素のモデルの正答率が 0.70 以上」**でなければ立たない
> (格子点のうち 134 以上は 150 / 175 / 200 / 300 / 500 / 999)。**現行規則より落ちやすい。**
> それでも根拠のない外挿域よりよい、という ADR-041 と同じ立場である。
> **★反映済**: `plans/PLAN-001` §4.1.1(**改訂ブロック 2 つ。打ち消し線 + 理由 + 日付**)/
> `configs/exp_phase1_main.yaml`(`theta: 0.70` / `theta_secondary: [0.65, 0.75]` /
> `shell_definition: null` / `shell_n_items: null`。**`extrapolation_radius` は引き続き `null`**)/
> `plans/PLAN-020` §7 / `logs/OPEN-ITEMS.md`(打ち消し線 + 新規 3 件)。
> **★新しく開いた人間待ち 3 件**(C2 は測定を一意に定めない):
> **(1) 殻の定義**(`R(M) \ R(1つ前の格子点)` か `R(M) \ R(99)` か)/
> **(2) 殻あたりの項目数と抽出経路**(現行の抽出のまま `predictions/` を切り直す = **追加 GPU 0** だが
> **`M = 100` の殻は 1,000 項目中およそ 20 項目で SE 約 0.10**。殻から 200 件引けば SE 約 0.014)/
> **(3) `θ = 0.70` の根拠**(ADR-041 決定2 が `#9` とは別に書くことを要求。**値は確定**)。
> **`pytest code/tests -q` = 906 passed。実験は 1 件も実行していない。凍結タグは打っていない。**

---

### 「次のアクション」(その28 〜 その30)

> **★★2026-09-09(その28・最新)。★F104 に案が付いた。残りは下の 6 本である。**
>
> 1. **★F104 を人間が決める**(`plans/PLAN-019` **§10.13.5**。記入欄は **4 行**)——
>    **F104-0**(2 つに分けてよいか)/ **F104-a**(`s2_item` の取得元)/
>    **F104-b**(`s2_tmpl` の取得元)/ **F104-c**(値そのもの)。
>    **★案 A(順6 からの移し替え)には「推奨しない」を付けた**(★F115)。
>    **案 B(格子)と案 C(1 点)の差はコストである** —— **B は本実行が k 倍
>    (62 → 186〜372 時間)、C は 62 時間のまま。エージェントはどちらも推していない**
> 2. ~~**★`θ` を人間が決める**~~ → **★2026-09-09(その30)決着(ADR-070)。**
>    **代わりに、C2 が開いた 2 件を人間が決める** —— **(a) 殻の定義**
>    (`R(M) \ R(1つ前の格子点)` か `R(M) \ R(99)` か)/ **(b) 殻あたりの項目数と抽出経路**
>    (現行抽出のまま切り直す = 追加 GPU 0 だが `M = 100` の殻は約 20 項目・SE 約 0.10 /
>    殻から 200 件引く = ADR-041 決定5 の変更・SE 約 0.014)。**順5 の残りブロッカーはこれである。**
>    **★`θ = 0.70` の根拠も未記入**(値は確定。順5 は回せる)
> 3. **★順6 を回す GPU を承認する。**Go/No-Go #0〜#3 はここでしか出ず、**段階 D の前に要る**
> 4. **★F114 の実行先を決める。****★F104-a の答えで所要時間が 62 → 372 時間まで動く**ので、
>    **1 の後に諮るのが自然である**(その27 の見立てのとおり)
> 5. **`code/analysis/primary.py`**(PLAN-016 §7-5)。**N5 が残っている。**
>    **★ADR-069 決定4 (2) は primary.py にも同じ形で効く**(実装時に適用する)
> 6. **`Documents/09_PAPER_PLAN.md` / `00_OVERVIEW.md:7` が再設計前のままである**(人間待ち)
...
> **★★2026-09-09(その30・最新)。PLANNER (Opus)。**
>
> **★やったこと**: **人間が `θ` の 4 行を採択したので ADR-070 を書き、5 ファイルに反映した。**
> **決定はすべて人間のものである**(エージェントは PLAN-020 §6 で案を並べただけ)。
> **★書き換えた先**: `logs/DECISIONS.md`(ADR-070)/ `plans/PLAN-001` §4.1.1(改訂ブロック 2 つ)/
> `configs/exp_phase1_main.yaml`(`theta` ほか 4 キー)/ `plans/PLAN-020` §7 / `logs/OPEN-ITEMS.md` /
> `logs/CHANGELOG.md` / 本ファイル。**`pytest` = 906 passed。**
> **★やっていないこと(意図的)**: **殻の実装をしていない**(未決 2 件が先。手を付けると
> 決定を先取りする)/ **`θ` の根拠を代筆していない**(規範的な線引き。`CLAUDE.md` §8)/
> **`eval.cells` を凍結していない**(B1 の判定値はこれ待ち)/ **実験を回していない**。
> **★次セッションが引き継ぐもの**: **★殻の定義 / 殻あたりの項目数と抽出経路**(順5 の残り)/
> **★`θ` の根拠** / **★F104**(`plans/PLAN-019` §10.13.5)/ **★F114 の実行先** /
...
## ★2026-09-09(その31b)にアーカイブへ移したブロック(ADR-063 運用規約1)

> **その31(前半)の「いま何をしているか」。その31b(ADR-071)が上書きした。**
> **1 文字も削っていない。**

> **★その31(前半)。ADR-070 決定4 が開いた 2 件の決定材料を作った**
> (`plans/PLAN-021-shell-measurement.md` + `plans/PLAN-021-check1/`)。**実装はしていない。**
> **★検査1**(`plans/PLAN-021-check1/`。**組合せ論の計数。実験結果ではない**)が 3 つ出した:
> **★F122** —— **経路 (a) の「追加 GPU 時間 0」は (b) に対する優位になっていない。**
> **順5 は 1 度も回っていないので、(b) も同じ 13,000 項目の 1 回の run で済む**(承認済 1.5h の中)。
> **しかも (a) 自身が ADR-041 決定5 の変更である** —— 決定5 の「**n は M 間で同一**」を破り、
> n は **19.8 件(`M=100`)〜 1,000 件(`M=25`)で 51 倍動く**。**差は GPU ではなく決定5 のどの条項を変えるかである。**
> **★F123** —— **定義 A も定義 B も、外挿腕が実際に使う組をほとんど含まない。**
> 主軸 3 水準目は `a>99` かつ `b>99`(★F120)だが、定義 B の殻に占める割合は
> **`M=150` で 5.1% / `M=200` で 8.4%** しかない。残りは負の被演算子と片側だけ域外の組である。
> → **案 C(`Q(M)` = 外挿腕の母集団そのもの)をエージェントが足した**(ADR-070 は A と B しか並べていない)。
> **★F124** —— **定義 B は台地アンカー 4 点(25/50/75/99)で殻が空になる。**
> ADR-041 決定3 規則2 は「小さい順に見て」θ を割る水準を探すので、**空の水準の扱いを決めないと規則2 が回らない。**
> → **「殻-c(空・極小の水準の扱い)」が新しい人間待ちとして開いた**(`logs/OPEN-ITEMS.md`)。
> **★補足**: `M = 100` の殻は**定義 A でも B でも 800 組**で同じ(1 つ前の格子点が 99 = 主域の半径)。
> **殻の定義では動かせない。**経路 (a) だと n = 19.8、SE = 0.103 である。
> **★書き換えた先**: `plans/PLAN-021-shell-measurement.md`(新設)/ `plans/PLAN-021-check1/`(新設)/
> `code/tests/test_design_facts.py`(**回帰テスト 6 件追加**)/ `logs/OPEN-ITEMS.md`(2 行に追記 + 1 行新設)/
> `logs/CHANGELOG.md` / 本ファイル。**`pytest code/tests -q` = 912 passed**(906 → 912)。
> **★やっていないこと(意図的)**: **殻を実装していない**(未決 3 件が先)/ **`θ` の根拠を代筆していない** /
> **`eval.cells` を凍結していない** / **実験を回していない**。

## ★2026-09-10(その32)—— STATE.md から移した 5 ブロック(ADR-063 運用規約1)

> **その31b の各節の最新ブロック。その32(ADR-071 の実装 + ★F125)が上書きした。**
> **1 文字も削っていない。**

### ヘッダ(最終更新。その31b)

最終更新: 2026-09-09(その31b)/ by PLANNER / CRITIC (Opus)
(**★★人間が `plans/PLAN-021` §6 の 3 行を一括採択した。ADR-071 を書いた**(D / (c) 限定版 / c3)。
**★人間の指示は「どれも AI の推奨案を採用する」であり、推奨案そのものをエージェントが作った** ——
**選択肢の比較検討はエージェントが行った。ADR-071 冒頭に明記してある**(ADR-039 決定3)。
**判定は `Q(M)` = `extrap_magnitude` の母集団**(★F123)/ **現行 13,000 項目に `Q(M)` の 7,000 項目を足す**
(**GPU 1.5 → 2.5h。10h の門は超えない**)/ **判定水準は `{125,150,175,200,300,500,999}`**。
**★外挿腕の生死は `Q(125)` と `Q(150)` が θ = 0.70 を超えるかだけで決まる。**
**★実装は次セッション**(`magnitude_sweep.py` / `sweep.py`。GPU 不要)。**順5 の残りは実装だけ。**
`pytest` = **914 passed**。**実験は 1 件も実行していない。`results/` は空。GPU 時間 0。**)
(**★ADR-070 決定4(C2)が開いた測定設計 2 件の決定材料を作った**(`plans/PLAN-021`。**決定 0 件**)。
**★F122: 経路 (a) の「追加 GPU 0」は (b) に対する優位ではない** —— 順5 は 1 度も回っていないので
(b) も同じ 13,000 項目の 1 回の run で済む。**しかも (a) は ADR-041 決定5 の「n は M 間で同一」を破る**(19.8 〜 1,000 件)。
**★F123: 定義 A も B も、外挿腕が使う組(`a>99` かつ `b>99`)を `M=150` で 5.1% しか含まない** → **案 C を足した**。
**★F124: 定義 B は台地アンカー 4 点で殻が空になり、規則2 が回らない** → **殻-c が新しく開いた**。
`pytest code/tests -q` = **912 passed**(+6)。**実験は 1 件も実行していない。`results/` は空。GPU 時間 0。**)

### 「いま何をしているか」(その31b)

> **★★2026-09-09(その31b・最新)。Phase 0。PLANNER / CRITIC。GPU 時間 0。**
> **★人間が `plans/PLAN-021` §6 の 3 行を一括採択した(ADR-071)。**
> **★指示は「どれも AI の推奨案を採用する。推奨案がない場合は全体を俯瞰して作製して」であった** ——
> **PLAN-021 §4 は推奨を付けていなかったので、推奨案そのものを ADR-071 で新たに作った。**
> **選択肢の比較検討はエージェントが行っている。ADR-071 冒頭に明記した**(ADR-039 決定3 / `CLAUDE.md` §8)。
> **決定1(殻-a = D)**: 判定は **`Q(M)`**(= `extrap_magnitude` の母集団)、報告は累積と定義 A を併記。
> **決定2(殻-b = (c) 限定版)**: 現行 13,000 項目を変えず、**`|Q(M)| >= 200` の 7 水準に `Q(M)` から
> 7,000 項目を足す**(計 20,000。**GPU 1.5 → 2.5h**)。**★決定5 に打ち消し線は要らない**(足す形のため)。
> **決定3(殻-c = c3)**: 判定水準 = **`{125,150,175,200,300,500,999}`**(凍結済の 200 件を `Q(M)` から
> 引ける水準)。**新しい数を作らない。**
> **★帰結: 外挿腕の生死は `Q(125)` と `Q(150)` が θ = 0.70 を超えるかだけで決まる**(規則2 + B1 + ★F120)。
> **★実装は次セッション**(`code/eval/battery/magnitude_sweep.py` / `code/eval/sweep.py`。**GPU 不要**)。
> **★反映済**: `logs/DECISIONS.md`(ADR-071)/ `configs/exp_phase1_main.yaml`
> (`shell_definition` / `shell_n_items` / `shell_radii` / `shell_judgement_radii` / `estimated_gpu_hours` 2.5)/
> `plans/PLAN-021` §6 / `logs/OPEN-ITEMS.md`(3 行に打ち消し線)。**`pytest` = 914 passed**(+2)。
> **★テストが実装前に罠を 1 つ潰した**: `(M-99)^2` は `M < 99` でも正になるので、
> **その式だけで判定水準を導くと台地アンカーが混じる。**config のコメントも直した。
>
> **★その31(前半)。ADR-070 決定4 が開いた 2 件の決定材料を作った**(`plans/PLAN-021` + `plans/PLAN-021-check1/`)。
> **★F122 / ★F123 / ★F124 の全文は `logs/STATE-ARCHIVE.md`「★2026-09-09(その31b)にアーカイブへ移したブロック」にある**(ADR-063 運用規約1)。

### ブロッカー1(その31b)

1. ~~**`θ`(外挿域の閾値)が未決である**~~ → **★2026-09-09(その30)決着(ADR-070)。**
   **A2 / 0.70 / B1 / C2 を人間が採択した。****★ただし順5 はまだ回せない** ——
   **C2(判定を殻に変える)が測定を一意に定めないため、新しい人間待ちが 2 件開いた**:
   **殻の定義**と**殻あたりの項目数・抽出経路**(`logs/OPEN-ITEMS.md` が正本)。
   **★ブロッカーの性質が変わった** —— 「規範的な線をどこに引くか」から
   **「殻をどう測るか」という測定設計**に移った。**`θ` の根拠(値ではない)も未記入である。**
   **★B1 の判定値は `eval.cells` の凍結待ちである**(134 はセル表から読んだ数。リテラルにしない)。
   **★★2026-09-09(その31b)決着(ADR-071)。人間が 3 行を一括採択した**(D / (c) 限定版 / c3)。
   **★順5 のブロッカーは人間の判断から実装へ移った** —— 残るのは
   `code/eval/battery/magnitude_sweep.py` と `code/eval/sweep.py` の実装だけである(**GPU 不要**)。
   **★`θ = 0.70` の根拠(値ではない)は依然として未記入である**(ADR-041 決定2 の要求)

### 「次のアクション」(その31)

> **★★2026-09-09(その31・最新)。順5 の残りは「殻をどう測るか」の 3 行だけである。**
>
> 1. ~~**★殻-a / 殻-b / 殻-c を人間が決める**~~ → **★★その31b 決着(ADR-071)。**
>    **代わりに実装する** —— `code/eval/battery/magnitude_sweep.py` に `Q(M)` からの抽出、
>    `code/eval/sweep.py` に 2 本の腕の測定と `metrics.json` の別ブロック、回帰テスト。
>    **GPU 不要。順5 の残りはこれだけである**(`plans/PLAN-021` §5 / ADR-071 帰結)
> 2. **★`θ = 0.70` の根拠を人間が書く**(ADR-041 決定2 の要求。**値は確定**)。
>    **規範的な線引きであり `CLAUDE.md` §8 の対象。エージェントは代筆しない**
> 3. **★F104 を人間が決める**(`plans/PLAN-019` **§10.13.5**。記入欄は **4 行**)——
>    案 B(格子)と案 C(1 点)の差はコストである(**62 → 186〜372 時間**)。
>    **★案 A には「推奨しない」を付けた**(★F115)
> 4. **★順6 を回す GPU を承認する。**Go/No-Go #0〜#3 はここでしか出ず、**段階 D の前に要る**
> 5. **★F114 の実行先を決める。****★F104-a の答えで 62 → 372 時間まで動く**ので、3 の後に諮る
> 6. **`code/analysis/primary.py`**(PLAN-016 §7-5)。**N5 が残っている。★ADR-069 決定4 (2) が効く**
> 7. **`Documents/09_PAPER_PLAN.md` / `00_OVERVIEW.md:7` が再設計前のままである**(人間待ち)
>

### 「引き継ぎ」(その31b)

> **★★2026-09-09(その31b・最新)。PLANNER / CRITIC (Opus)。**
>
> **★やったこと**: (1) ADR-070 決定4 が開いた 2 件の決定材料 `plans/PLAN-021` を作り、
> 検査1 で **★F122 / ★F123 / ★F124** を出した。(2) **人間が §6 の 3 行を一括採択したので
> ADR-071 を書き、config と `logs/OPEN-ITEMS.md` に反映した**(D / (c) 限定版 / c3)。
> **★指示は「どれも AI の推奨案を採用する」であり、推奨案そのものをエージェントが作った。**
> **選択肢の比較検討はエージェントが行っている。ADR-071 冒頭に明記した**(ADR-039 決定3)。
> **★やっていないこと**: **殻の実装**(次セッション。GPU 不要)/ **`θ` の根拠の代筆**(`CLAUDE.md` §8)/
> **`eval.cells` の凍結** / **実験** / **ポッドの起動**。**`pytest` = 914 passed。**
> **★次セッションが引き継ぐもの**: **★殻の実装**(`magnitude_sweep.py` / `sweep.py` / 回帰テスト。
> **順5 の残りはこれだけ**)/ **★`θ` の根拠** / **★F104**(`plans/PLAN-019` §10.13.5)/
> **★F114 の実行先** / **順6 の GPU 承認** / **N5 と `code/analysis/primary.py`** /
> **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`** / **★L(d)** / **★C** / **★E** / **E-5 (b)** / **★2** /
> **PLAN-018 §4.3**。

## ★2026-09-10(その33)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **その32 の各節の最新ブロック。その33(ADR-072。★F125 の決着)が上書きした。**
> **1 文字も削っていない。**
...
### 「repo の状態」の ⚠️ ★F125 行(その32。ADR-072 で解決)
...
## ★2026-09-10(その34)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その34 で各節を新しいブロックに差し替えたため。
...
## ★2026-09-10(その35)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その35 で各節を新しいブロックに差し替えたため。
...
## ★2026-09-10(その37)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その37 で順5 の結果を報告し、各節を新しいブロックに差し替えたため。
...
### 「人間の承認・判断を待っている事項」索引表の決着済み 7 行(その37。ADR-067 / 068 / 069)

> 移した理由: `STATE.md` が 403 行になり目標 400 行を超えたため(ADR-063 運用規約5・6)。**1 文字も削っていない。**
...
## ★2026-09-10(その38)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その38 で人間が ADR-074(`M*` = 999 / ★F126 = 案 (ii) / ★F127 を開く)を採択し、各節を新しいブロックに差し替えたため。`...` は間の行を省いた印であり、省いた行は `STATE.md` に残っている。

### ヘッダ(最終更新。その37)

最終更新: 2026-09-10(その37)/ by RUNNER (Opus)
(**★順5 の掃引 run `20260910_104249_sweep_m` の結果を読み、4 値分解を「わかっていること」に載せた。解釈はしていない。**
**★`parse_fail` の大半はパーサ規則2 が同じ答えの言い直しを落としたものだった(仕様どおり)→ ★F126 として人間に上げた。**
**`M*` は人間が置く。GPU は使っていない。ポッドはすべて停止中。`results/` は空。**)

### 「いま何をしているか」(その37)

> **★★2026-09-10(その37・最新)。Phase 0。RUNNER (Opus)。順5 の結果を報告した。GPU・ポッドは触っていない。**
> - `runs/20260910_104249_sweep_m/metrics.json` の 3 ブロック(`quadrant` / `by_radius` / `grid_shell`)を読み、
>   **4 値の合計がすべての水準・すべてのシードで 1.0** であることを確かめた → 「わかっていること」の先頭
> - **`CLAUDE.md` §7 の点検**で `predictions/`(開発機・git 管理外)を件数だけ数えた → **★F126**(`logs/OPEN-ITEMS.md`)
> - `logs/OPEN-ITEMS.md` の停止中ポッドの行に `omjvbdanmbrzc8` を足した
>
> **★その36 の記録は `logs/STATE-ARCHIVE.md`「その37」にある**(ADR-063 運用規約1)。

### 「わかっていること」順5 の見出しと `rule` 26 件の行(その37)

### ★順5 の桁数掃引 [run:20260910_104249_sweep_m](2026-09-10 その37 に読んだ。**解釈・`M*` はまだ無い**)
...
- **素のモデルで `rule`(= `a+b+2`)に当たった応答が 26 件ある**(腕1 のみ。すべて負の被演算子を含む項目。腕2 は 0)。`other_error` だけが高い水準は無い

### 「現在のブロッカー」1 の末尾 3 行(その37)

   **★2026-09-10(その36)に掃引 run `20260910_104249_sweep_m` が完走し、その37 に結果を報告した**(「わかっていること」の先頭)。
   **次は人間が `M*` を置く。その前に ★F126(パーサ規則2 を見直すか)を決める必要がある**(`logs/OPEN-ITEMS.md`)
   **★`θ = 0.70` の根拠(値ではない)は依然として未記入である**(ADR-041 決定2 の要求。順5 は回せる)

### 「現在のブロッカー」検出力分析の 4 行(その37)
...
### 「人間の承認・判断を待っている事項」★F126 の索引行(その37)

| **★F126** ★新(その37) | **順5 の `parse_fail` の大半は、パーサ規則2(PLAN-001 §5.4.1)が同じ答えの言い直しを落としたもの**(仕様どおり)。**`M*` を置く前に規則2 を見直すか**(案 (i)〜(iii)。追加 GPU 0) | **`M*` の前** |
...
### 「Phase 0 に必要な段階」段階 C の行(その37)

| **C** | GPU 小(`none` モデルのみ。FT は 1 本も回さない)= 順5 の桁数掃引 / Go-No-Go #0 〜 #3 | ~~**`θ` 待ち + GPU 承認待ち**~~ → ~~★F125 待ち~~ → **順5: 完走(run `20260910_104249_sweep_m`。その36)・結果は報告済(その37)。★F126 と `M*` は人間待ち。**順6: GPU 承認待ち |

### 「次のアクション」(その37)

> **★★2026-09-10(その37・最新)。順5 の結果は報告済み。次は ★F126 と `M*`(どちらも人間)である。**
>
> 1. **★F126 を人間が決める**(`logs/OPEN-ITEMS.md`。`M*` を置く前にパーサ規則2 を見直すか。(ii) なら PLAN + ADR → 再採点。**追加 GPU 0**)
> 1b. **停止中ポッドの terminate を人間が決める**(`zxwdkgxutbuoph` / `46pggs1odwb09r` / `omjvbdanmbrzc8`。`logs/OPEN-ITEMS.md`)
> 2. **★`θ = 0.70` の根拠を人間が書く**(ADR-041 決定2 の要求。**値は確定**)。エージェントは代筆しない
> 3. **★`M*` を人間が置く**(★F126 の後。`quadrant` に ADR-041 規則2 を当てる。ADR-041 / ADR-045。**エージェントは置かない**)
> 4. **★F104 を人間が決める**(`plans/PLAN-019` **§10.13.5**。記入欄は **4 行**)
> 5. **★順6 を回す GPU を承認する**(Go/No-Go #0〜#3 はここでしか出ず、**段階 D の前に要る**)
> 6. **★F114 の実行先を決める**(4 の後)
> 7. **`code/analysis/primary.py`**(N5 待ち)/ **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`**(人間待ち)/
...
## ★2026-09-11(その39)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その39 で PLAN-022 手順1〜6 を実装し、ADR-075 を採択して、各節を新しいブロックに差し替えたため。

### 「冒頭」の旧ブロック(その38)

最終更新: 2026-09-10(その38)/ by RUNNER (Opus)
(**★人間が ADR-074 を採択した: `M*` = 999(格子の上端で打ち切り)/ ★F126 = 案 (ii)(パーサ規則2 を「後ろの整数がすべて同じ値なら採る」に変える)/ ★F127 を開く。**
**実装(パーサ・再採点・`M*` の config 反映)は `plans/PLAN-022` に切った。次セッション。GPU 0。**
**GPU は使っていない。ポッドはすべて停止中(terminate は人間の操作待ち)。`results/` は空。**)

### 「いま何をしているか」の旧ブロック(その38)

> **★★2026-09-10(その38・最新)。Phase 0。RUNNER (Opus)。人間の決定を記録した。GPU・ポッドは触っていない。**
> - 順5 の腕1・腕2 と ★F126 の判断の仕方を人間に説明し、エージェントの案を人間が一括採択した → **ADR-074**
>   (**提案 エージェント / 採択 人間**。決定1 `M*` = 999 / 決定2 ★F126 = (ii) / 決定3 ★F127 / 決定4 ポッドは人間が terminate)
> - `predictions/` を件数だけ数えた: `rule` 26 件のうち 24 件は被演算子 −1 → **★F127**(`logs/OPEN-ITEMS.md`)
> - **`plans/PLAN-022-parser-rule2-unanimous.md` を起草した**(パーサ規則2 の変更 + 再採点 + `M*` の config 反映。**未実装**)
>
> **★その37 の記録は `logs/STATE-ARCHIVE.md`「その38」にある**(ADR-063 運用規約1)。

### 「次のアクション」の旧ブロック(その38)

> **★★2026-09-10(その38・最新)。`M*` と ★F126 は決着した(ADR-074)。次は PLAN-022 の実装である。**
>
> 1. **★`plans/PLAN-022` を実装する**(IMPLEMENTER。`logs/HANDOFF.md`。パーサ規則2 → テスト → 再採点を別の run として → `M*` を config へ。**GPU 0**。§5.1 の検査 C1〜C5 が通るまで数値を報告しない)
> 1b. **停止中ポッドの terminate は人間がコンソールで操作する**(ADR-074 決定4)。ボリューム `r963j7swke`(HF トークン入り)の扱いと HF トークンの revoke も人間が決める
> 2. **★`θ = 0.70` の根拠を人間が書く**(ADR-041 決定2 の要求。**値は確定**)。エージェントは代筆しない
> 3. **★F127 を人間が決める**(被演算子 −1 / 真値 −1 の除外。`logs/OPEN-ITEMS.md`。凍結前)
> 4. **★F104 を人間が決める**(`plans/PLAN-019` **§10.13.5**。記入欄は **4 行**)
> 5. **★順6 を回す GPU を承認する**(Go/No-Go #0〜#3 はここでしか出ず、**段階 D の前に要る**)
> 6. **★F114 の実行先を決める**(4 の後)
> 7. **`code/analysis/primary.py`**(N5 待ち)/ **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`**(人間待ち)/
...
> **★★2026-09-10(その38・最新)。RUNNER (Opus)。context-guard の警告(約 120k トークン)で切った。**
>
> **★やったこと**: 順5 の腕1・腕2 と ★F126 の判断の仕方を人間に説明 → 人間がエージェントの案を一括採択 → **ADR-074** /
> `logs/OPEN-ITEMS.md`(★F126 決着・★F127 新設・`M*` 決着・ポッドの行)/ **`plans/PLAN-022` を起草**。
> **★やっていないこと**: **パーサの変更・再採点・`M*` の config 反映(すべて PLAN-022。`extrapolation_radius` は `null` のまま)** /
> ポッドの terminate(人間)/ **★F127 の除外そのもの** / `θ` の根拠の代筆。GPU・ポッドは触っていない。コード変更なしなので pytest は回していない。
> **★次セッションが引き継ぐもの**: **PLAN-022 の実装(`logs/HANDOFF.md`)** / 停止中ポッドの terminate(人間)/ **★`θ` の根拠** / **★F127** /
> **★F104** / **★F114 の実行先** / **順6 の GPU 承認** / **N5 と `code/analysis/primary.py`** / **`09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`** /
> **★L(d)** / **★C** / **★E** / **E-5 (b)** / **★2** / **PLAN-018 §4.3**。

## ★2026-09-11(その40)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その40 で PLAN-022 §5 の再採点を回し、ADR-075 を実装して、各節を新しいブロックに差し替えたため。
> あわせて「repo の状態」から、経緯の記録(2026-08-22 の編集記録など。運用規約2)と、2026-08-27 の行(本実行が通る)と食い違う古い行(運用規約6)を移した。

### 「冒頭」の旧ブロック(その39)

最終更新: 2026-09-11(その39。夜間の自律セッション)/ by IMPLEMENTER (Opus)
(**★人間の事前一括承認(2026-09-10 22:47「AI の提案はエージェントが正しいと判断したものはすべて承認」)の下で ADR-075(★F127 = 被演算子 −1 を評価項目から外す)を採択した。コードへの反映は未実施。**
**PLAN-022 は手順1〜6 まで済(パーサ規則2 / `M*` = 999 を config へ。commit `bb5f112`)。再採点(§5)は `code/eval/rescore.py` の途中版(未レビュー・未実行)が残っている。**
**GPU は使っていない。ポッドはすべて停止中(terminate は人間の操作待ち)。`results/` は空。**)

### 「いま何をしているか」の旧ブロック(その39)

> **★★2026-09-11(その39・最新)。Phase 0。IMPLEMENTER (Opus)。夜間の自律セッション(人間は就寝中。06:00 まで)。GPU・ポッドは触っていない。**
> - **PLAN-022 手順1〜6 を実装した**(commit `bb5f112`。`unanimous_integer` / `M*` = 999 を `configs/exp_phase1_main.yaml` へ / run と照合する回帰テスト / PLAN-001 §4.1.1・§5.4.1 改訂)
> - **PLAN-022 §5(再採点)は途中**: サブエージェントが `code/eval/rescore.py`(約 700 行)と `code/tests/test_rescore.py`(11 passed)を書いたところで利用制限で止まった。**`execute()` に書きかけがある。本物の run には一度も掛けていない**
> - **ADR-075 を採択**(★F127。提案 エージェント / 採択 人間の事前一括承認)。**`EXCLUDED_OPERANDS` の変更は未実装**
> - 順6 の準備状況を監査した → `plans/PLAN-023-order6-readiness.md`(**監査結果のみ。計画は未起草**)
>
> **★その38 の記録は `logs/STATE-ARCHIVE.md`「その39」にある**(ADR-063 運用規約1)。

### 「わかっていること」順5 の §7 の点検の旧ブロック(その37 記入。★F126 / ★F127 の決着前)

**`CLAUDE.md` §7 の点検(エージェント。`predictions/` を件数だけ数えた)**:
- **`parse_fail` の大半はパーサの取りこぼしである**(PLAN-001 §5.4.1 規則2 が「同じ答えの言い直し」を落とす。**仕様どおり**)。腕2 の 160 件中 133、腕1 の 2,258 件中 1,264 は、最後の印の後ろの整数が**すべて真値と同じ値**だった。`p2` 規則値と同じ値の件数は両腕とも 0 → **★F126**
- **腕1 の低下は負の被演算子に集中している**: 両方 ≥ 0 は correct 0.982(3,206 件)/ 片方 < 0 は 0.750(other_error 0.080・parse_fail 0.168。6,446 件)/ 両方 < 0 は 0.641(parse_fail 0.335。3,348 件)
- **素のモデルで `rule`(= `a+b+2`)に当たった応答が 26 件ある**(腕1 のみ。すべて負の被演算子を含む項目。腕2 は 0)。`other_error` だけが高い水準は無い。**★その38: 26 件のうち 24 件は被演算子の片方が −1(「−1 の符号を落とした和」は `a+b+2` と同じ数)/ 1 件は (11, −12) で真値 −1 に 1 / 1 件 (2, −254) は未説明。応答文は読んでいない → ★F127**
- **M=125 は母集団 676 組から 1,000 回引いているので、シード間で組が重なる**(異なる組 563)。温度 0 なので同じ組は同じ応答になる → **この水準のシード間 SD は独立な 5 標本の SD ではない**
- `dirty: true` と `git_diff.patch` 0 バイトは食い違っていない: `code/artifacts.py:230-236` は dirty を `git status --porcelain`(未追跡を含む)、差分を `git diff HEAD`(追跡のみ)で取る。**追跡ファイルの変更は無かった。**未追跡が run ディレクトリ自身だったかどうかは未検証

### 「repo の状態」pytest の行の旧版(その39)

| `pytest code/tests -q` → **967 passed**(2026-09-11 その39 実測。未コミットの `test_rescore.py` を除く)。~~40~~ → ~~227~~ → ~~427~~ → ~~589~~ → ~~831~~ → ~~838~~ → ~~839~~ → ~~874~~ → ~~898~~ → ~~900~~ → ~~912~~ → ~~914~~ → ~~956~~ → ~~957~~ → **967**(PLAN-022 のパーサ境界事例 +9・`M*` の照合 +1) | `code/tests/` |

### 「repo の状態」T1 / T2 の行の旧版
...
### 「repo の状態」から移した行(経緯の記録 = 運用規約2 / 2026-08-27 の「本実行が通る」と食い違う古い行 = 運用規約6)
...
1. ~~**`θ`(外挿域の閾値)が未決である**~~ → ADR-070(その30)→ ~~殻の測り方 3 行~~ → ADR-071(その31b)→
   ...
   **★2026-09-10(その36)に掃引 run `20260910_104249_sweep_m` が完走し、その37 に結果を報告した**(「わかっていること」の先頭)。
   **★その38 に人間が `M*` = 999 と ★F126 = (ii) を決めた(ADR-074)。残るのは `plans/PLAN-022` の実装(パーサ・再採点・config。GPU 0)** → **その39 にパーサと config は済(`bb5f112`)。残るのは再採点(§5)**
   **★`θ = 0.70` の根拠(値ではない)は依然として未記入である**(ADR-041 決定2 の要求。`M*` を置く妨げにはしない)

### 「人間の承認・判断を待っている事項」冒頭の注記の旧版(その39)

**★残っているのは ★F104(`s2_item` / `s2_tmpl` の取得元)と ★F114 の実行先である。**
**この 2 本と `M*`(順5)が、`power_sim.py` の本実行を止めている全部である。**

### 「人間の承認・判断を待っている事項」から落とした ★F127 の行(ADR-075。2026-09-11 実装済)

| 記号 | 決めること | 期限 |
|---|---|---|
| ~~**★F127**~~ | ~~被演算子 −1 / 真値 −1 の項目も評価項目から外すか~~ → **★2026-09-11 決着(ADR-075。事前一括承認)。−1 は外す / 真値 −1 は外さない** | **実装が残る** |

### 「Phase 0 に必要な段階」C の行の旧版(その39)

| **C** | GPU 小(`none` モデルのみ。FT は 1 本も回さない)= 順5 の桁数掃引 / Go-No-Go #0 〜 #3 | ~~**`θ` 待ち + GPU 承認待ち**~~ → ~~★F125 待ち~~ → **順5: 完走(run `20260910_104249_sweep_m`。その36)・結果は報告済(その37)。`M*` = 999(ADR-074)。PLAN-022(パーサ・再採点)が残る。**順6: GPU 承認待ち |

### 「次のアクション」の旧ブロック(その39)

> **★★2026-09-11(その39・最新)。次は PLAN-022 §5 の再採点を仕上げ、ADR-075 を実装する。**
>
> 1. **★`code/eval/rescore.py` を読んで仕上げ、順5 を別の run として採点し直す**(PLAN-022 §5・§5.1 の C1〜C5。`logs/HANDOFF.md`)
> 2. **★ADR-075 を実装する**(`code/data_gen/pool.py` の `EXCLUDED_OPERANDS` = {1, −1}。件数を固定した回帰テストと PLAN-001 の除外の記述を直す)
> 3. **★`plans/PLAN-023`(順6 の準備)を起草する**(`eval.batteries` / `eval.cells` / `pool_items` → `fill_cells` / `pool_seed` / 反復生成 / 順6 の config / dry-run。GPU 0)
> 4. **E-5 (b)**(K の出どころを `metrics.json` に焼き込む。本実験の前だけ安い。`logs/OPEN-ITEMS.md`)
> 5. 人間: 停止中ポッドの terminate / **★`θ` の根拠** / **★F104** / **★F114 の実行先** / **順6 の GPU 承認** / N5 / `09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`

### 「引き継ぎ」の旧ブロック(その39)

> **★★2026-09-11(その39・最新)。IMPLEMENTER (Opus)。context-guard の警告(約 210k トークン)と利用制限で切った。**
>
> **★やったこと**: PLAN-022 手順1〜6(`bb5f112`)/ ★F127 の応答文 26 件を読み、ADR-075 を事前一括承認の下で採択 / `logs/OPEN-ITEMS.md` の ★F127 を閉じた / 順6 の準備状況の監査(PLAN-023)。
> **★やっていないこと**: 再採点の実行(`rescore.py` は途中版)/ ADR-075 のコード反映 / PLAN-023 の起草 / `θ` の根拠の代筆 / GPU。
> **★次セッションが引き継ぐもの**: `logs/HANDOFF.md`(夜間の自律作業の続き。06:00 まで)/ 上の「次のアクション」1〜4 / 人間待ちは `logs/OPEN-ITEMS.md`。

---

## ★2026-09-11(その41)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない。**移した理由: その41 で `plans/PLAN-023`(順6 の準備)を起草し、各節を新しいブロックに差し替えたため。
> 「現在のブロッカー」の 2 と「人間の承認・判断を待っている事項」の表は行を足しただけなので、移したものは無い。

### 「冒頭」の旧ブロック(その40)

最終更新: 2026-09-11(その40。朝。夜間の自律モードは終わった)/ by IMPLEMENTER (Opus)
(**PLAN-022 は完了した**(再採点 run `20260910_215422_rescore_sweep_m`。C1〜C5 すべて pass)。**ADR-075 は人間が朝に承認し、実装した**(`EXCLUDED_OPERANDS` = {1, −1})。
**2026-09-10 22:47 の事前一括承認は終わった。以降の設計判断は通常どおり人間の承認を得る。**
**GPU は使っていない。ポッドはすべて停止中(terminate は人間の操作待ち)。`results/` は空。**)

### 「いま何をしているか」の旧ブロック(その40)

> **★★2026-09-11(その40・最新)。Phase 0。IMPLEMENTER (Opus)。人間は起床済み(夜間の自律モードは終わった)。GPU・ポッドは触っていない。**
> - **PLAN-022 は完了した**: `code/eval/rescore.py` をレビューして仕上げ(commit `760f29b`)、順5 を別の run `20260910_215422_rescore_sweep_m` として採点し直した(commit `fab3e1b`。**C1〜C5 すべて pass。解釈はしていない**)
> - **ADR-075 を実装した**(人間が朝に承認。commit `5967594`。`EXCLUDED_OPERANDS` = {1, −1})
> - **次は `plans/PLAN-023`(順6 の準備)の起草**(いまは §1 の監査結果だけ)
>
> **★その39 の記録は `logs/STATE-ARCHIVE.md`「その40」にある**(ADR-063 運用規約1)。

### 「repo の状態」プールの行の旧版(その40 以前)
...
### 「Phase 0 に必要な段階」C の行の旧版(その40)

| **C** | GPU 小(`none` モデルのみ。FT は 1 本も回さない)= 順5 の桁数掃引 / Go-No-Go #0 〜 #3 | ~~**`θ` 待ち + GPU 承認待ち**~~ → ~~★F125 待ち~~ → **順5: 完走(run `20260910_104249_sweep_m`。その36)・結果は報告済(その37)。`M*` = 999(ADR-074)。PLAN-022 も済(再採点 run `20260910_215422_rescore_sweep_m`。その40)。**順6: GPU 承認待ち(準備は `plans/PLAN-023`。未起草) |

### 「次のアクション」の旧ブロック(その40)

> **★★2026-09-11(その40・最新)。PLAN-022 と ADR-075 の実装は済んだ。次は PLAN-023(順6 の準備)の起草である。**
>
> 1. **★`plans/PLAN-023-order6-readiness.md` を起草する**(§1 は監査結果。§2 の決めることを埋める: `eval.batteries` / `eval.cells` / `pool_items` → `fill_cells` / `pool_seed` / 反復生成 / 順6 の config / dry-run。GPU 0。**決めることは案として書き、人間に上げる**)
> 2. **E-5 (b)**(K の出どころを `metrics.json` に焼き込む。本実験の前だけ安い。`logs/OPEN-ITEMS.md`)
> 3. 人間: 停止中ポッドの terminate / **★`θ` の根拠** / **★F104** / **★F114 の実行先** / **順6 の GPU 承認** / N5 / `09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`

### 「引き継ぎ」の旧ブロック(その40)

> **★★2026-09-11(その40・最新)。IMPLEMENTER (Opus)。PLAN-022 を終えた(1 セッション = 1 PLAN)うえ、コンテキストが約 10 万トークンを超えたので切った。**
>
> **★やったこと**: `rescore.py` のレビューと仕上げ(`760f29b`)/ 再採点 run `20260910_215422_rescore_sweep_m`(C1〜C5 pass。`fab3e1b`)/ ADR-075 の実装(人間が朝に承認。`5967594`)。
> **★やっていないこと**: PLAN-023 の起草 / E-5 (b) / `θ` の根拠の代筆 / GPU。

---

## ★2026-09-11(その42)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない**(`git show 4895376:STATE.md` から機械的に切り出した)。移した理由: その42 で ADR-076(★F128〜★F137・E-5 (b)・順6 の GPU 承認)が決まり、各節を新しいブロックに差し替えたため。

### 「冒頭」の旧ブロック(その41)

最終更新: 2026-09-11(その41)/ by PLANNER (Opus)
(**`plans/PLAN-023`(順6 の準備)を起草した。決定は 0 件。**人間に上げる点を ★F128〜★F137 として `logs/OPEN-ITEMS.md` に置いた(記入欄は PLAN-023 §2.6)。
**コードも config も変えていない。GPU は使っていない。ポッドはすべて停止中(terminate は人間の操作待ち)。`results/` は空。**)

### 「いま何をしているか」の旧ブロック(その41)

> **★★2026-09-11(その41・最新)。Phase 0。PLANNER (Opus)。GPU・ポッドは触っていない。コードと config は変えていない。**
> - **`plans/PLAN-023` を起草した**: §1.1 に追加の監査 13 件(A1〜A13)/ §2 に 4 項目と E-5 (b) の「現状 / 案 / 人間に上げる点」/ §2.6 に記入欄 / §4 に実装手順 / §5 に dry-run のコマンド
> - **人間に上げたのは ★F128〜★F137 の 10 件**(`logs/OPEN-ITEMS.md`「★2026-09-11(その41)の追記」)。**いちばん重いのは ★F128**(#2 の一覧を縛るのが `none` か `ident` か。文書のあいだの食い違い)
> - **次は人間の記入(PLAN-023 §2.6)→ IMPLEMENTER が §4 手順1〜5**
>
> **★その40 の記録は `logs/STATE-ARCHIVE.md`「その41」にある**(ADR-063 運用規約1)。

### 「現在のブロッカー」2 の旧 2 行(その41)

2. **順6 を回す GPU の承認が下りていない。****事前登録の凍結(順9)はこれが出るまで打てない。**
   **★2026-09-11(その41): 準備の計画 `plans/PLAN-023` は起草済。GPU の前に ★F128〜★F137 の決定が要る(プールの生成に効く 6 件が先)。**

### 「人間の承認・判断を待っている事項」の旧行 ★F128〜★F137(その41)

| **★F128〜★F137** ★新(その41) | **順6 の準備(`plans/PLAN-023` §2.6 の記入欄)**: #2 の一覧を縛るもの / プール main か pilot / タスク4〜6 の形 / 閾値オフセット / `pool_seed` / 実装判断 3 件。**プールの生成に効く 6 件(F128・F129・F131・F135・F136・F137)が先** | プール生成の前 / GPU の前 |

### 「それ以外の人間待ち」の旧行(その41。順6 と E-5 (b) に打ち消し線を足す前)

~~**`θ`**~~(**ADR-070 で決着**)→ ~~**★殻の定義** / **★殻あたりの項目数と抽出経路**~~(**ADR-071 で決着。2026-09-10 実装済**)/ ~~**★★F125**~~ / ~~**★ADR-071 の実装判断 3 件**~~(**ADR-072 で決着**) / **★`θ` の根拠** / **順6 と本実験 40 run の GPU 構成** / **E-5 の案 (b)**(期限あり)/

### 「Phase 0 に必要な段階」段階 C の旧行(その41)

| **C** | GPU 小(`none` モデルのみ。FT は 1 本も回さない)= 順5 の桁数掃引 / Go-No-Go #0 〜 #3 | ~~**`θ` 待ち + GPU 承認待ち**~~ → ~~★F125 待ち~~ → **順5: 完走(run `20260910_104249_sweep_m`。その36)・結果は報告済(その37)。`M*` = 999(ADR-074)。PLAN-022 も済(再採点 run `20260910_215422_rescore_sweep_m`。その40)。**順6: `plans/PLAN-023` は起草済(その41)。★F128〜★F137 の記入と GPU 承認待ち |

### 「次のアクション」の旧ブロック(その41)

> **★★2026-09-11(その41・最新)。PLAN-023 を起草した。次は人間の記入、その後に実装である。**
>
> 1. **人間: `plans/PLAN-023` §2.6 の記入**(★F128〜★F137。**プールの生成に効く 6 件が先**)
> 2. **IMPLEMENTER: PLAN-023 §4 手順1〜5**(config → `pool.py` → `eval_pool.py` → `run.py --dry-run` → `code/analysis/gonogo.py`)と §5 の dry-run。**記入の前に始めない**
> 3. 人間: **順6 の GPU 承認**(文面は PLAN-023 §2.4)/ **E-5 (b)**(期限は順6 の前に早まった)/ 停止中ポッドの terminate / **★`θ` の根拠** / **★F104** / **★F114 の実行先** / N5 / `09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`

### 「引き継ぎ」の旧ブロック(その41。先頭 4 行。5 行目は据え置き)

> **★★2026-09-11(その41・最新)。PLANNER (Opus)。PLAN-023 の起草を終えた(1 セッション = 1 PLAN)うえ、コンテキストが約 10 万トークンを超えたので切った。**
>
> **★やったこと**: PLAN-023 の §1.1(追加の監査)/ §2(4 項目 + E-5 (b))/ §2.6(記入欄)/ §4(実装手順)/ §5(dry-run)/ `logs/OPEN-ITEMS.md` に ★F128〜★F137。
> **★やっていないこと**: 実装 / config の変更 / 決定 / `θ` の根拠の代筆 / GPU。

---

## ★2026-09-11(その43)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない**(`git show 6c6e113:STATE.md` から機械的に切り出した)。移した理由: その43 で PLAN-023 を完了し(ADR-077 を含む)、各節を新しいブロックに差し替えたため。
...
### 「repo の状態」の pytest の行(その40 の 980)
...
### 「repo の状態」の ⚠️ プールはサンプリングしていない の行
...
## ★2026-09-11(その44)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない**(`git show a285834:STATE.md` から機械的に切り出した)。移した理由: その44 で順6 の GPU を起動し(R1〜R5 のチェーンがポッド上で実行中)、各節を新しいブロックに差し替えたため。
...
### 「わかっていること」順5 の桁数掃引(旧規則2)の節(その44 に移した)

### ★順5 の桁数掃引 [run:20260910_104249_sweep_m](2026-09-10 その37 に読んだ。**解釈はしていない。`M*` = 999 は ADR-074 で人間が置いた**)

素の `meta-llama/Llama-3.1-8B-Instruct`(revision `0e9e39f…` / adapter null / 参照規則 `p2` / `direct` / `do_sample: false`・温度 0 /
chat template / バッチ 4 / RTX 4090)。各水準 5 シード × 200 = 1,000 項目。**4 値の合計は全ブロック・全水準・全シードで 1.0。**

**腕2 = `quadrant`(`Q(M)` = 両被演算子 ∈ [100, M]。`roles` = `judgement_input`)。**`rule` の SD は全水準 0、`other_error` の SD は 300 / 500 で 0.0022、他は 0

| M | \|Q(M)\| | 異なる組 / 1,000 | correct | rule | other_error | parse_fail | SD correct | SD parse_fail |
|---|---|---|---|---|---|---|---|---|
| 125 | 676 | 563 | 0.991 | 0.000 | 0.000 | 0.009 | 0.0102 | 0.0102 |
| 150 | 2,601 | 852 | 0.998 | 0.000 | 0.000 | 0.002 | 0.0027 | 0.0027 |
| 175 | 5,776 | 937 | 0.991 | 0.000 | 0.000 | 0.009 | 0.0074 | 0.0074 |
| 200 | 10,201 | 966 | 0.987 | 0.000 | 0.000 | 0.013 | 0.0057 | 0.0057 |
| 300 | 40,401 | 989 | 0.980 | 0.000 | 0.001 | 0.019 | 0.0100 | 0.0089 |
| 500 | 160,801 | 998 | 0.951 | 0.000 | 0.001 | 0.048 | 0.0185 | 0.0199 |
| 999 | 810,000 | 1,000 | 0.940 | 0.000 | 0.000 | 0.060 | 0.0117 | 0.0117 |

**腕1 = `by_radius`(`R(M)` = [−M, M]² から一様。記述)。**correct の SD は 0.0175〜0.0391。定義 A の格子殻(記述)は `metrics.json` の `grid_shell`(M=100 の殻は 23 項目)

| M | 25 | 50 | 75 | 99 | 100 | 110 | 125 | 150 | 175 | 200 | 300 | 500 | 999 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| correct | .860 | .842 | .823 | .807 | .801 | .794 | .786 | .758 | .776 | .774 | .711 | .713 | .680 |
| rule | .009 | .007 | .003 | .002 | .001 | .000 | .001 | .001 | .001 | .000 | .001 | .000 | .000 |
| other_error | .016 | .026 | .023 | .031 | .033 | .033 | .037 | .051 | .051 | .066 | .071 | .070 | .083 |
| parse_fail | .115 | .125 | .151 | .160 | .165 | .173 | .176 | .190 | .172 | .160 | .217 | .217 | .237 |

**timing**: 合計 6,250.1 秒(重みの読み込み 95.7 / 生成 6,147.4)/ 20,000 項目 / 0.307 秒/項目。log.txt に WARN・ERROR・Traceback は 0 件

**`CLAUDE.md` §7 の点検(その37。数値は旧規則2 で数えたもの。★F126 は ADR-074 と下の再採点、★F127 は ADR-075 で決着した。点検の全文は `logs/STATE-ARCHIVE.md`「その40」)**:
- **腕1 の低下は負の被演算子に集中している**: 両方 ≥ 0 は correct 0.982(3,206 件)/ 片方 < 0 は 0.750(other_error 0.080・parse_fail 0.168。6,446 件)/ 両方 < 0 は 0.641(parse_fail 0.335。3,348 件)
- **M=125 は母集団 676 組から 1,000 回引いているので、シード間で組が重なる**(異なる組 563)。温度 0 なので同じ組は同じ応答になる → **この水準のシード間 SD は独立な 5 標本の SD ではない**

---

## ★2026-09-12(その45)—— STATE.md から移したブロック(ADR-063 運用規約1)

> **1 文字も削っていない**(`git show 0c57599:STATE.md` から機械的に切り出した)。移した理由: その45 で順6 の読みの判断材料(`plans/PLAN-024`)を書き、各節を新しいブロックに差し替えたため。

### 冒頭の「最終更新」(その44)

最終更新: 2026-09-11(その44)/ by RUNNER (Opus)
(**★順6 の GPU が完走した(R1〜R5 = 7,520 項目・回)。5 run を回収・コミットし、ポッド `omjvbdanmbrzc8` を停止した(EXITED)。**
**Go/No-Go の表は `results/gonogo_order6/gonogo.json`。印は付いたが判断は人間。強制選択に ★F138 を見つけて OPEN-ITEMS に上げた。**)

### 「いま何をしているか」(その44)

> **★★2026-09-11(その44・最新)。Phase 0。RUNNER (Opus)。順6 の GPU は完走し、回収・コミット・ポッド停止まで済んだ(ADR-076 決定11)。**
> - ポッド `omjvbdanmbrzc8`(EUR-IS-1 / RTX 4090 SECURE $0.74/時)を人間の選択で start(14:12:42Z)。/workspace に重み・HF トークン・venv が残っていた
> - bundle で repo を `a285834` に更新 → データ再生成(評価プールの差分 0 行)→ preflight は 5 run とも FAIL 0 → チェーンで R1〜R5
> - **1 回目のチェーンは R2 の途中で人間がポッドを止めた**(R1 は完了済み)。人間の指示で再 start(16:00:40Z)し R2〜R5 を回し直した(16:46:23Z CHAIN_DONE)。
>   途中の R2 は `/workspace/pod_moved_aside/` へ退避(消していない)。**16:47Z 頃に stop → EXITED**(2 回目の稼働 2,835 秒。1 回目の停止時刻は人間の操作で、エージェントは知らない)
> - 数値は下の「わかっていること ★順6」。**★F138 を `logs/OPEN-ITEMS.md` に上げた**
>
> **★その43 の記録は `logs/STATE-ARCHIVE.md`「その44」にある**(ADR-063 運用規約1)。

### 「現在のブロッカー」2 の先頭 2 行(その44)

2. **★2026-09-11(その44): 順6 は完走した**(5 run 回収・コミット済・ポッド停止済)。**いま止めているのは人間の判断である** ——
   Go/No-Go #1〜#3 の読み / 落ちたセルの一覧 / T3・T1b の扱い(★F138・ADR-047 決定2)/ R4 の突き合わせの合否(ADR-040)/ Δ の値。**事前登録の凍結(順9)はこの後。**

### 「次のアクション」(その44)

> **★★2026-09-11(その44・最新)。順6 は完走した。次は人間の判断である。**
>
> 1. **人間: 順6 の表を読む**(先に PLAN-023 §6.1 の読み 6 点)→ Go/No-Go #1〜#3 / 落ちたセルの一覧 / **★F138**(強制選択の真値が向きで決まる)/ R4 の 16 件(ADR-040 決定3)/ `cost.txt`
> 2. その後: Δ の 5 行(ADR-069 決定2 の移し替え)→ 事前登録の凍結(順9)
> 3. 人間: 停止中ポッドの terminate / **★`θ` の根拠** / **★F104** / **★F114 の実行先** / **Phase 1 本実験 40 run の GPU 構成** / N5 / `09_PAPER_PLAN.md` / `00_OVERVIEW.md:7` / **PLAN-023 §6.1 の読み(順6 の表を読む前)**

### 「引き継ぎ」(その44)

> **★★2026-09-11(その44・最新)。RUNNER (Opus)。順6 を完走させ、回収・コミット・ポッド停止まで済ませた。コンテキストは約 25 万トークン(人間の指示で続行した)。**
>
> **★やったこと**: ポッド start(2 回。2 回目は人間の指示)/ データ再生成 / R1〜R5 / 回収・検証・コミット(run ごと)/ gonogo・compare_runs / ★F138 / ポッド停止(EXITED)。
> **★やっていないこと**: Go/No-Go の判断・解釈 / `cost.txt`(人間)/ push / terminate(人間)。
> **★次セッションが引き継ぐもの**: `logs/HANDOFF.md` / 上の「次のアクション」/ 人間待ちは `logs/OPEN-ITEMS.md`。

---

## ★2026-09-12(その46)—— STATE.md から移したブロック(ADR-063 運用規約1)

> その46 の PLANNER が `STATE.md` の 4 ブロックを差し替えたときに、ここへ機械的に移した(1 文字も削っていない)。

### 冒頭の「最終更新」(その45)

最終更新: 2026-09-12(その45)/ by PLANNER (Opus)
(**★順6 の読みの判断材料を `plans/PLAN-024` に並べた(D1〜D7。決定は 0 件・推奨なし)。新しい事実 ★F139(T1b の Yes/No 質量)と
★F140(パーサの取りこぼし)を OPEN-ITEMS に上げた。GPU・ポッドは触っていない。**)

### 「いま何をしているか」(その45)

> **★★2026-09-12(その45・最新)。Phase 0。PLANNER (Opus)。順6 の結果を人間が読むための材料を作った。GPU 0。**
> - **`plans/PLAN-024`**: 事実(§1。すべて run_id 付き)/ 判断の順番(§2)/ D1〜D7 の選択肢と「どの文書が変わるか」(§3)/ 記入欄(§4)。**推奨は付けていない**
> - **★事前登録を字面どおりに当てはめると、二値群 6 セルが外れて主要検定の df は 6 → 2 になる**(ADR-076 決定1 + ADR-047 決定2 + ADR-064 決定2)。
>   **ただし ★F138 / ★F139 で「測り直す」側を選ぶ余地を残すなら、6 セルの扱い(D4)は先に確定しない**(PLAN-024 §2)
> - `logs/OPEN-ITEMS.md` の 3 行に PLAN-024 への参照を足し、★F139 / ★F140 を新設した
>
> **★その44 の記録は `logs/STATE-ARCHIVE.md`「その45」にある**(ADR-063 運用規約1)。

### 「次のアクション」(その45)

> **★★2026-09-12(その45・最新)。判断材料は揃った。次は人間が `plans/PLAN-024` §4 に記入する。**
>
> 1. **人間: PLAN-024 §4 の D1〜D7**(順番は §2。**D2・D3 を D4 より先に**)。記入後、エージェントが ADR に起こす(提案 エージェント / 採択 人間)
> 2. 選ばれた案に GPU が要れば(D2 (c) / D3 (b)(c) / D5 (a))、PLAN を切って GPU の承認を取る
> 3. その後: Δ の 5 行(ADR-069 決定2 の移し替え。**移し替え元の T2 の場面間分散は 0**。PLAN-024 §1.7)→ 事前登録の凍結(順9)
> 4. 人間: 停止中ポッドの terminate / **★`θ` の根拠** / **★F104** / **★F114 の実行先** / **Phase 1 本実験 40 run の GPU 構成** / N5 / `09_PAPER_PLAN.md` / `00_OVERVIEW.md:7`

### 「引き継ぎ」(その45)

> **★★2026-09-12(その45・最新)。PLANNER (Opus)。順6 の読みの判断材料(PLAN-024)を書いた。GPU 0・ポッドは触っていない。**
>
> **★やったこと**: R1〜R5 の成果物を読み取りだけで点検(極性別の応答・Yes/No 質量・R4 の 16 件の差・parse_fail の中身・R4 の gonogo 表)/ PLAN-024 / OPEN-ITEMS の 3 行に参照 + ★F139・★F140。
> **★やっていないこと**: 解釈・推奨・閾値の変更・落ちたセルの一覧の確定・T1b / T3 を外す決定 / GPU / push / `cost.txt`(人間)。
> **★次セッションが引き継ぐもの**: `plans/PLAN-024` §4(人間の記入)/ 上の「次のアクション」/ 人間待ちは `logs/OPEN-ITEMS.md`。

---

## ★2026-09-12(その47)—— STATE.md から移したブロック(ADR-063 運用規約1)

> その47 の PLANNER が `STATE.md` の 4 ブロックを差し替えたときに、ここへ機械的に移した(1 文字も削っていない)。
