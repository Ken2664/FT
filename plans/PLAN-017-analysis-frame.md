...
> **表の組み方は主要検定の説明変数そのものを作る。**`coverage` の付け方が変われば
> `task:coverage` の交互作用が別物になるので、人間が選ぶ。

- 作成日: 2026-09-07
- 最終更新: 2026-09-07(その16)
- ステータス: **`草案`**(**決定 0 件。人間のレビュー待ち**)
...
- [ ] **§5 の E-1 〜 E-7 に人間の決定がある**(ADR に記録。提案者と採択者を分ける)
- [ ] `pytest code/tests -q` が通る
- [ ] **§8 の回帰テストが入っている**
- [ ] **`--dry-run` が件数だけを出して成功する**
- [ ] `logs/CHANGELOG.md` と `STATE.md` を更新した
- [ ] **主軸から落ちた行の件数が記録に残る**
