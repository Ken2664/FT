...
> **★2026-09-11(その42)に人間が 12 行すべてを記入した。すべて (a)。**正本は **ADR-076**(`logs/DECISIONS.md`)。
> 記入はチャットの選択肢で行い、IMPLEMENTER (Opus) がこの表へ転記した(提案 = エージェント(その41)/ 採択 = 人間。ADR-039 決定3)。
> **GPU の承認は条件付き**: プールの manifest をコミットし、§5 の dry-run が通った後のセッションで回す。
